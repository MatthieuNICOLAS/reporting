...
- MUTE : Implémenter le mécanisme de renommage
  ...
  - Réflexion sur l'exécution d'une opération distante de renommage
    ...
    - Garbage collection d'opérations
      - Dans *LogootSplit*, j'ai besoin de conserver une opération tant qu'un renommage en concurrence peut être généré
	...
	- De cette manière, toute la logique des opérations et de leurs transformations est encapsulée dans /mute-structs/
	- Et la seule responsabilité de /mute-core/ est de transformer les opérations avant de les délivrer
*** Planned

...
- BD2 : Préparer CM5
  ...
  - Factorisation de code
** Semaine du <2020-12-14 Mon> au <2020-12-18 Fri>
*** Planned
**** DONE Algo : Préparer TD10
- Correction
  - L'exo recherche dichotomique me paraît intéressant à faire avec eux
  - Pareil pour un 1er exo de tri
**** DONE Papier : Rédiger section sur /LSEQ/
**** DONE Papier : Rédiger conclusion
**** DONE Papier : Refaire une passe sur le papier avant envoi à Gérald et Olivier
- Histoire de vérifier si y a pas des faux-raccords dans le contenu que j'ai ajouté
- Peut aussi en profiter pour remplacer cette figure sur le temps d'intégration des opérations de renommages qui a un problème d'échelle
  - Voir ce que donnerait un tableau à la place
*** Done
- Algo : Préparer TD10
  - Charles m'a contacté concernant cette prochaine séance
    - Souhaite donner le DM3 dans le courant de la semaine
    - Faut donc avoir couvert les pré-requis du DM cette semaine
    - Càd la recherche dichotomique, algorithmes de tri et les objets
  - Correction
    - Correction
      - Recherche dichotomique
      - Tri par sélection
    - Cours sur les objets
      - Principe
	- Paradigme de programmation désormais courant dans les langages
	- Permet de définir de nouveaux types
	- qui vont représenter des concepts (Personne, Voiture, Compte bancaire)
	- Possède un état
	  - Décrit par l'ensemble de ses attributs
	- Et peut effectuer un certains nombres d'actions/d'opérations
	  - Décrites par ses méthodes
      - Constructeur __init__
	- Permet de construire un exemplaire de l'objet
	- On parle d'instance
      - Paramètre /self/
      ...
      - __str__
      - Avantages
	- Permettra de généraliser puis de spécialiser des comportements à l'aide de l'héritage
	- Peut être utilisé en tant que boîte noire par d'autres programmes
- Papier : Rédiger section sur /LSEQ/
  - Du mal à voir comment je peux introduire le doublement de base
- Meeting
  - Réunion avec Gérald et Olivier pour faire le point
    - Maintenant que j'ai fini le gros des cours, peut me concentrer sur la recherche de nouveau
    - Reprise de l'écriture du papier sur la version distribuée de mon algo de renommage
      - A ajouté la partie Related Work et Conclusion
      - A envoyé cette nouvelle version à Gérald et Olivier
	- Retours normalement courant de semaine pro
      - Reste maintenant le refactoring de la partie background/début contribution par rapport aux retours que j'ai déjà eu de leur part
    - Problème étant où l'envoyer ?
      - ICDCS'21
	- Abstract le 6 janvier, papier le 13
	- En l'état, pas atteignable
	- Mais si deadline reportée, ça se tente
      - PODC'21
	- Abstract le 11 février, papier le 15
	- Mais mon papier est probablement pas assez théorique
      - TPDS
	- Journal
	- Faudrait retravailler le papier pour inclure la partie PaPoC
      - On en rediscute la semaine de la rentrée en fonction de mon avancée
    - A discuté de la suite du programme
      - Rédaction du manuscrit
      - Travail sur la proposition de stratégies pour la génération de
	- Définir les données qui peuvent être pris en considération pour la décision de génération d'un renommage
	- Proposer des stratégies à partir de celles-ci
	- Identifier avantages/inconvénients de ces stratégies
	  - Potentiellement une agressive, une conservatrice
** Semaine du <2020-12-21 Mon> au <2020-12-25 Fri>
*** Planned
**** DONE Algo : Corriger DM2
- Ça serait pas mal de faire ça avant les vacs
- Histoire qu'iels soient fixé-e-s et moi débarrassé
*** Done
** Semaine du <2021-01-04 Mon> au <2021-01-08 Fri>
*** Planned
**** DONE Algo : Préparer TD11
*** Done
- Algo : Préparer TD11
  - Consiste à revoir et terminer le TD10
  - Correction de l'exo 1 en live
    - Jusqu'à la question e)
    - Permet ainsi de voir la définition de classe, l'instanciation, les méthodes spéciales
  - Rappel sur le DM3
    - Insister sur lectures des consignes
    - Notamment, par rapport à ce que prend en entrée/paramètre/argument et ce que retourne les fonctions demandées
    - Si on vous demande d'écrire une fonction qui prend une ligne du fichier donné en paramètre et qui renvoie un objet Ville
      - Si vous m'écrivez une fonction qui prend autre chose en paramètre ou retourne autre chose, c'est incorrect
	- Même si ça marche
	- Vous faites du hors-sujet
    - Et lorsque l'entrée n'est pas précisée, prenez le temps de réfléchir
      - Dans tous les cas, on précise ce qu'on attend en sortie
      - Reste à déterminer de quelles données vous avez besoin pour produire ce résultat
- Répétition Quentin
  - Début à 10:13
  - Slide 5
    - Mot-clés positifs alors que tu veux dire le contraire
  - Slide 7
    - Pas forcément clair le lien entre les différentes étapes IMO
  - Slide 11
    - C'est quoi un processus "marqué" ?
      - Contraire d'un processus aléatoire
  - Slide 12/13
    - Pas trop clair ton schéma de structure en étoile je trouve
  - Slide 15
    - Différence entre Hub et inter-organisationnelle ?
  - Slide 28
    - Faire apparaître les différentes couleurs sur l'image de la page HAL aussi ?
  - Slide 32-34
    - Redonner la sémantique de ces paramètres ?
  - Slide 40
    - Paraît étrange les 2 1ers points par rapport au titre de la slide
  - Problème au niveau de l'affichage de tes slides ?
    - Affiche souvent plusieurs étapes d'un coup, mais en plusieurs étapes
    - C'est normal ?
  - Choix des thetas
    - Pour apparition d'un lien, d'un hub ou d'un inter-organisationnelle ?
    - Comment ce choix s'est fait ?
...
- Meeting
  - Bonne année
  - Réunion avec Olivier et Gérald pour discuter du papier sur RLS
    - ICDCS n'a pas repoussé sa deadline, donc c'est mort
    - Dorénavant vise TPDS
      - Journal et plus une conf, doit revoir le format du papier
      - Idée est de fusionner l'article pour PaPoC et celui que j'ai commencé à rédiger pour avoir un article complet sur RLS centralisé puis distribué
    - On se revoit dans une dizaine de jours pour en rediscuter
    - J'avais envoyé avant les vacances une version du papier
      - M'ont fait part de quelques remarques/questions
      - M'envoient la liste complète des corrections à apporter dans l'aprem
  - Assisté à la soutenance blanche de Quentin
    - Pas grand chose à redire, ça me paraît bien
** Semaine du <2021-01-11 Mon> au <2021-01-15 Fri>
*** Planned
**** CANCELLED Papier : Rédiger section sur /LogootSplit/
**** CANCELLED Papier : Déplacer section sur /RenamableLogootSplit/ dans *Background*
...
**** DONE Papier : Retrouver références pour mécanisme d'anti-entropie
- À l'heure actuelle, je ne cite aucune référence lorsque j'indique qu'on utilise un mécanisme d'anti-entropie
- On avait avec Victorien et Gérald retrouvé des références à ce sujet pour un livrable d'OpenPaaS
- Voir pour retrouver et ajouter ces références au papier
**** DONE Cours : Contacter Vincent pour infos supplémentaires sur le projet Informatique
- Questions à poser
  - Sur emploi du temps, j'ai rien vendredi aprem mais je vois que lui a les 2I de 13 à 17h, est-ce normal ?
  - Merci pour le mail à Claude, sa réponse aide un peu à voir ce qui est attendu des étudiants
  - Mentionne un sujet sur sa page dans son mail à Tesnim, quelle page ?
    - J'ai fait un tour sur vdespre.free.fr mais j'ai rien vu qui correspondrait
  - Infos que je n'ai pas
    - C'est quoi le but du projet ?
    - Volume horaire ?
    - Date ?
    - Rendus intermédiaires ?
    - Y a des exemples de projets des années passées ?
**** DONE Papier : Relire *Logoot-Undo: Distributed Collaborative Editing System on P2P Networks*
- A été publié dans TPDS
- Permettra de voir le format/les termes employés
- Dispo ici : https://ieeexplore.ieee.org/document/5374392
**** DONE Cours : Préparer slides sur UML et les diagrammes de classe
- La 1ère séquence du projet doit se conclure sur l'élaboration, au propre, du diagramme de classe du projet
- Et de la mise en place du squelette de classes correspondant sur un repo git
- Pour le moment, on a juste vu l'utilisation du diagramme de classe dans la conception de base de données
- Préparer quelques slides pour rappeler formalisme et expliquer utilisation dans la conception orientée objet
- Différences majeures
  - Types des attributs
  - Associations se traduisent en attributs, non plus en relation
  - Méthodes
  - Correspondance entre diagrammes et classes
- Insister sur intérêt/utilisation
**** DONE Papier : Prendre en compte remarques d'Olivier
- Olivier m'a envoyé ses remarques/notes
- Faire les modifs en conséquences
*** Done
- Cours : Contacter Vincent pour infos supplémentaires sur le projet Informatique
  - Il s'agit du projet dispo à l'adresse suivante : http://vdespre.free.fr/Downloads/Java2A/Projet.pdf
  - Afin d'en discuter, a planifié une réunion mercredi 13
    - Soit à 13h30, soit à 16h en fonction des dispos de Tesnim
  - C'est à 13h30
  - Questions à aborder
    - C'est quoi le but du projet ?
      - D'après le message de Claude, on évalue
	- analyse des besoins
	- qualité de la modélisation (classes, relations, contraintes)
	- organisation du logiciel
	- Eléments de sûreté : proc. stockées, transactions, triggers, sauvegardes ...
	- qualité de l'interface
	- validation par rapport aux besoins
      - Est-ce toujours ça ?
    - C'est quoi l'objectif à atteindre ?
      - Le sujet a l'air plutôt libre
      - Qu'est-ce qu'ils doivent rendre forcément, quels éléments composent la base de ce qui est attendu ?
    - Volume horaire ?
    ...
    - Rendus intermédiaires ?
    - Interface admin ?
      - Pour ajouter/modifier évènements par exemple
      - Pourrait alors avoir 2 applis, une utilisateur, une admin
    - Quid de la POO ?
      - Projet porté sur la conception orientée objet
      - Important ?
	- Pas vu l'objet en PHP
	- Peut leur filer des ressources à ce sujet
    - Gestion de version, utilisation de git ?
    - Cours sur Arche ?
  - Réunion
    - 15 étudiants
    - 7 groupes (6 groupes de 2, un seul trinôme)
    - 2 groupes doivent faire du Java pur
    - 2 autres doivent faire la BDD
    - Volume horaire
      - 50h
      - Environ 3-4h par semaine
    - Préfère la partie en client lourd
    - Interface graphique
      - awt
    - Javadoc/Documentation
      - Demandé
    - Jalons explicites
      - Réunion une fois par semaine
    - Conception orientée objet
      - Main doit faire une ligne
    - Soutenance
      - Courant fin mai/début juin
    - Rendus
      - Pas de rapport
      - Code + documentation
      - Mais une présentation
    - Évaluation
      - Si travail honnête, 12
      - Voir réguliarité, bonne volonté, s'ils jouent le jeu
    - Conseils
      - Faire réunion à la fin et au début de chaque séquence pour faire le point et les relancer
    - 1ère séquence
      - Peut partir sur conception orientée objet
	- Objectif de la séquence est de faire le diagramme de classe au propre + squelette correspondant sur un git
    - 2ème séquence
      - Code
    - 3ème séquence
  ...
  - Cours
    - P3
      - Design, diagramme de classe
      - Préparer les packages, fichiers
      - Prise en main de git
    - P4
      - Code + début de doc
      - Vise à avoir un projet fonctionnel à l'issue de cette période
    - P5
      - Fini le code
      - Met la documentation en forme
      - Prépare soutenance
    - Rendu
      - Accès au code, Javadoc et slides
    - Organisation
      - Binômes et 1 trinôme
      - Niveau homogène
      - Utilisation de git
	- Règle : Pas de travail de 2 étudiants en même temps sur le même fichier
	- Pas pusher code qui compile pas
      - 4h par semaine
      - Heures dans l'EDT
	- Heures prévues pour bosser le projet
	- Peuvent être utilisées pour contacter et discuter avec encadrants
    - Jeu doit être graphique ?
      - 1ère phase : jeu qui fonctionne, en texte
      - 2ème phase : jeu avec une interface graphique (AWT)
	- Peut récupérer images (créditer artiste ?)
      - 3ème phase : Sauvegarde en BD
      - Phase 1 et 2 par tout le monde
      - Phase 3 pour seulement quelques groupes
	- Demande pas mal de taf supplémentaires
    - Questions
      - Droit à d'autres librairies ?
	- Pour UI, pourquoi pas
	- Mais sinon, à demander et gérer au cas par cas
      - Droit à Unity ?
	- Au cas par cas
      - Droit au langage ?
	- Si à l'aise
	...
	- Vincent
      - Giroux - Greve
	- Tesnim
      - Seznec - Thouvenin - Maimbourg
	- M'en occuperait
      - Bluem - Noel
	- Vincent
      - Champseix - Masschelein
	- M'en occuperait
      - Brouillard - Soria
	- Tesnim
      - Schoumacher - Terrien
...
- Papier : Prendre en compte remarques d'Olivier
  - Majoritairement des corrections
  - Quelques annotations que je ne suis pas sûr de comprendre dont il faudra rediscuter
    - 6.1
      - ?
    - 7.1
      - Orientation ?
      - Est-ce bien le bon terme ?
  - Autres correcitons où je suis pas sûr
    - Début page 7 : storing -> storage of => *the* storage of ?
    - 6.1, page 7 : in -> given => ça marche dans ce cas-là ?
    - 7.2, page 8 : once again -> as in the previous approach => as *with* the previous approach ?
- Papier : Relire *Logoot-Undo: Distributed Collaborative Editing System on P2P Networks*
  - Contenu
    - Undo naïf consiste généralement à générer une opération inverse à l'opération initiale
    - Ne fonctionne généralement pas
      - Exemple du undo d'une suppression
      - Si multiples undo concurrents, texte réinséré plusieurs fois
    - Propose d'associer un degré aux différents éléments de la séquence
      - Par défaut, 1, indique que la ligne est visibile
      - Si supprimé ou /undo/ de l'/insert/, passe à 0
      - Peut devenir négatif si plusieurs /undos/ ou /removes/ en concurrence
      - /redo/ incrémente le degré
      - En cas de /undos/ concurrents, force autant de /redos/ pour faire ré-apparaître la ligne
	- Les /undos/ concurrents ont un effet accumulatif et non pas idempotent
    - Ajoute une structure de données à l'état, le /cemetery/
      - Permet de ne pas polluer l'état du document avec ce degré
      - Ou de ne pas forcer le parcours du log des opés avant l'affichage du document
      - Utilisée uniquement pour stocker les lignes ayant un degré < 0
	- Les lignes avec 1 sont présentes dans le document, donc pas besoin de les reproduire
	- Les lignes avec 0 sont juste absentes du doucment, donc pas besoin de les reproduire
	- Uniquement besoin de gérer les autres cas
  - Remarques sur papier dans TPDS par rapport à ICDCS
    - Papier de Logoot publié à ICDCS 2009 dispo ici : https://hal.inria.fr/inria-00432368/
    - Reprend des paragraphes entiers pour *Background*, *Related Work*
      - Parfois développe juste ces paragraphes
    - Dans section *Editing a Logoot-Undo document*
      - Re-présente Logoot
      - Re-présente la /Random identifier allocation strategy/
      - Mais introduit la /Boundary identifier allocation strategy/
    - Section *Modifying a Logoot-Undo document...*
      - Nouvelle section
    - Développe la sous-section *Correctness of the approach* en une section complète dans TPDS
      - Retravaille la preuve de l'/eventual consistency/ pour Logoot-Undo
      - Ajoute une preuve de la préservation de l'intention
      - Ajoute une analyse et comparaison de la complexité de Logoot-Undo par rapport à WOOT et MOT2
    - La partie évaluation a l'air d'être peu ou prou la même
      - La présentation des résultats a l'air d'avoir un peu changé
      - De nouveaux articles (/random articles/) ont été ajoutés à l'évaluation
- Papier : Remarques sur papiers de TPDS
  - J'ai l'impression que les papiers ont une composante théorique/formelle prononcée
  - Les 3 papiers que j'ai regardé embarque une preuve de leur correction
  - Le papier de Logoot-Undo a été complété par rapport à celui de Logoot avec une analyse de sa complexité
  - Seul finalement le papier de Logoot-Undo présente une évaluation expérimentale
  - Est-ce juste une mauvaise pioche ou ça correspond à la ligne éditoriale du journal ?
- Papier : Retrouver références pour mécanisme d'anti-entropie
  - Il s'agissait du L2.2 *Implantation du composant middleware de réplication hybride pair-à-pair V1*
    - Dispo ici : https://research.linagora.com/download/attachments/16580772/SP2_L2-2_ImplantationDuComposantMiddlewareDeReplicationHybridePairAPair_VF.pdf?version=1&modificationDate=1493975898000
  - La référence qu'on avait citée est *Detection of Mutual Inconsistency in Distributed Systems*
    - Abstract dispo ici : https://dl.acm.org/doi/10.1109/TSE.1983.236733
  - J'ai pris le temps de voir par quels papiers cet article est cité, histoire de trouver d'autres papiers qui pourraient m'intéresser
    - On retrouve quelques papiers de la littérature CRDTs
      - *A Scalable Conflict-Free Replicated Set Data Type*
	- Par Andrei Deftu et Jan Griebsch en 2013
	- Dispo ici : https://ieeexplore.ieee.org/document/6681588
      - *A Conflict-Free Replicated JSON Datatype*
      - *Delta state replicated data types*
      - *DottedDB: Anti-Entropy without Merkle Trees, Deletes without Tombstones*
	- Me fait penser qu'il faudrait que je relise un peu sur les mécanismes d'anti-entropie basés sur les Merkle Trees pour me rafraîchir la mémoire à ce sujet
    - On retrouve d'autres papiers qui ont l'air de proposer des améliorations
      - *Version stamps-decentralized version vectors*
	- Par Paulo Sérgio Almeida, Carlos Baquero et Victor Fonte en 2002
	- Argumente que le design de base des /version vectors/ pose problème en cas de partition
	  - Comment choisir son identité ? (si je comprends bien)
	- Propose un mécanisme de capture de la causalité alternatif
	- Peut-être l'ancêtre de *Interval Tree Clock* en fait
	- Dispo ici : https://ieeexplore.ieee.org/document/1022304
      - *Concise version vectors in WinFS*
	- Par Dahlia Malkhi et Doug Terry en 2007
	- Présentent /predecessor vectors with exceptions (PVEs)/
	- Permettrait de réduire l'overhead (metadata et network) dû aux /version vectors/
	- Dispo ici : https://link.springer.com/article/10.1007/s00446-007-0044-y
      - *Interval Tree Clocks*
	- Par Paulo Sérgio Almeida, Carlos Baquero et Victor Fonte en 2008
      - *Log' version vector: Logging version vectors concisely in dynamic replication*
	- Par Hyun-Gul Roh, Myeongjae Jeon, Euiseong Seo, Jinsoo Kim et Joonwon Lee en 2010
	- Propose /log-prime/, un nouvel encodage du /version vector/ qui aurait une taille fixe, composée de 3 entrées
	- Sacrifierait un peu de précision, mais éviterait mécanisme de GC du /version vector/
	- Dispo ici : https://www.sciencedirect.com/science/article/abs/pii/S0020019010001158
      - *Adaptive Version Clocks and the OffSync Protocol*
	- Par Christoph P. Neumann, Andreas M. Wahl et Richard Lenz en 2012
	- A l'air d'adapter les /version vectors/ de façon à les rendre viable dans un système dynamique
	- Dispo ici : https://ieeexplore.ieee.org/document/6280383/
      - *Scalable eventually consistent counters over unreliable networks*
	- Par Paulo Sérgio Almeida et Carlos Baquero en 2017
	- Semble proposer un compteur dont l'état ne croît pas linéairement avec le nombre de nodes
	- Se base sur le principe des /Handoff Counters/
	- L'approche serait générique
	- Dispo ici : https://link.springer.com/article/10.1007%2Fs00446-017-0322-2
      - *Causality Tracking Trade-offs for Distributed Storage*
	- Par Hugo Guerreiro, Luis Rodigues, Nuno Preguiça et Nivia Quental en 2020
	- A plutôt l'air de comparer les avantages et inconvénients, en fonction de la situation, des différentes approches possibles pour capturer la causalité entre évènements
	- Dispo ici : https://ieeexplore.ieee.org/document/9306734
    - Les modifs ont l'air de porter sur le mécanisme de /causality tracking/, pas sur le mécanisme d'anti-entropie en soi
    - À creuser
- Cours : Préparer slides sur UML et les diagrammes de classe
  - Plan
    - Conception Orientée Objet dans le cadre du Projet informatique
      - N'évalue pas uniquement votre code en tant que tel dans le cadre de ce projet
      - Mais allons aussi évaluer l'architecture logicielle de votre projet
      - Concrètement, comment votre programme est construit ?
    - Intuition
      - Il a plusieurs façons d'écrire/de construire un programme donné
      - Certaines sont meilleures que d'autres
	- Décomposer votre programme en objets qui interagissent entre eux >>> une seule classe
      - On appelle cet art la Conception Orientée Objet
    - Qu'est-ce que la Conception Orientée Objet ?
      - Ensemble de bonnes pratiques sur comment construire une application
      - Repose notamment sur des principes de la Programmation Orientée Objet (polymorphisme, encapsulation)
    - But de la Conception Orientée Objet
      - Rendre le code plus compréhensible (séparation des responsabilités)
      - Permettre de le rendre plus maintenable (factorisation du code)
      - Permettre de le rendre plus évolutif (minimisation des dépendances)
    - Outil
      - Idée est de réfléchir au code avant de l'écrire
      - Pour appuyer cette réflexion, des outils existent
      - Y en a qu'on connaît déjà et qu'on a déjà utilisé ensemble
    - Diagrammes de classe UML
      - Utilisés en cours de BD pour la conception de bases de données
      - Nous permettait de représenter les différentes entités de notre système et les associations entre ces dernières
      - De là, on proposait un schéma relationnel de notre BD
      - Mais à la base, designé pour la conception orientée objet
    - Rappel Diagrammes UML
      - Classe
      - Attributs
      - Opérations
      - Exemple avec Point
      - Correspondance avec du code
      - Associations
    - Quelques différences par rapport au cours de BD
      - Ici, les opérations (méthodes) nous intéressent
      - Les associations ne vont pas se traduire en de nouvelles classes
	- Exemple avec Rectangle
	- Mais en des attributs
	- À vous de déterminer quel objet doit référencer/connaître l'autre
	- S'indique à l'aide du sens de la flèche
      - Visibilité des attributs et méthodes est importante
    - Rappel héritage
      - Exemple avec Animal, Chien et Poisson
	- Ajouter Nourriture, Croquette et Granulé ?
      - Classes filles sont des spécialisations, classe mère une généralisation
      - Puis mettre Animal en abstract
    - Présentation interface
      - Exemple avec Iterable, List, SuiteFibonacci
    - Notion de dépendance
      - Les différents types d'associations (simple, héritage, implémentation) traduisent une dépendance entre les objets
      - La dépendance a une conséquence concrète
	- Si A dépend de B
	- Si je modifie B
	- A peut être cassé
    - Exemple avec Point et Rectangle
      - Dépendance correspond au sens de la flèche
      - Si modifie Point
	- Je modifie la signature du constructeur de Point (besoin de fournir un z dorénavant)
      - Alors Rectangle est cassé
      - Pareil pour seTranslater()
      - Si par contre je modifie Rectangle
      - Aucun impact sur Point
    - Dépendance et encapsulation
      - Un de nos objectifs est de maîtriser et de limiter ces dépendances
      - Un moyen d'y parvenir est d'utiliser l'encapsulation
	- Rendre privé les attributs et méthodes
	- Si les autres classes ne connaissent pas/n'ont pas accès aux attributs et méthodes, alors peuvent être modifiés sans risques
	- Réduit pas le nombre d'associations dans notre diagramme de classe
	- Mais réduit la surface du code concerné par la relation de dépendance
    - Conclusion sur dépendance
      - Utile aussi pour déterminer ordre d'écriture des classes
	- Ne peut pas écrire Rectangle tant que je n'ai pas Point
	- Les classes sans dépendances doivent être les 1ères écrites
      - Utile pour répartir le travail
	- Permet de décomposer notre programme en groupes de classes, sans dépendances (ou peu) entre ces groupes
	- Peut répartir ces groupes de classes entre les membres du projet pour travailler sans se marcher dessus
    - Diagrammes de classe en pratique
      - Pour démarrer, faire diagrammes sur papier
      - Une fois que diagramme stable, pourra utiliser un logiciel pour le dessiner
	- PlantUML
    - Questions ?
- Meeting
  - A cours à 14h, donc je m'eclipserai probablement vers 50
  - Globalement, ça va
    - J'avais une surveillance d'examen, donc ça m'a donné une occasion de sortir de chez moi et de croiser mes étudiants, ce qui a fait un peu de bien
  - Concernant mon travail de cette semaine
    - J'en parlais la semaine dernière, vu que ICDCS n'a pas repoussé sa deadline, on a décidé de retravailler notre article sur RenamableLogootSplit pour le soumettre à TPDS
    - Cette semaine, j'ai (re-)jeté un oeil à différents articles publiés à TPDS (Logoot-Undo notamment)
      - Histoire d'essayer de voir sur quels aspects mon article pourrait pécher actuellement
    - Sinon j'ai pris en compte les corrections que m'a suggéré Olivier
    - Là je suis en train de fusionner le papier pour PaPoC et le draft pour ICDCS au format du journal et de poursuivre sa rédaction
  - Je profite de ce message pour me renseigner, est-ce qu'il y a la possibilité de pouvoir aller de manière régulière au labo pour bosser ?
    - Si j'ai bien compris, y a moyen de retourner sur place par exemple pour des réunions
    - Mais est-ce possible par exemple de pouvoir travailler au labo 1 jour par semaine ?
    - Histoire de casser un peu ma routine
    - Et de baisser ma facture de chauffage
  - Réunion
    - François
      - 2 nouveaux docteurs dans l'équipe /o/
      - Oui y a moyen d'aller au labo
	- Pas besoin de prendre RDV
	- Mais faut se concerter avec ses collègues de bureau
    - Abir
      - Occupée avec les cours
    - Alexandre
      - Essaie d'obtenir les dumps de Wikipedia via l'API ou le code d'Abdul
    - Anis
      - Effectue les corrections proposées par Olivier et François
      - Avance sur l'implem (composition de services si je me souviens bien)
      - Doit fournir une preuve de sa participation en entreprise pour "échapper" au reste des formations
    - Claudia
      - Soutenance Le s'est bien passée
      - Assisté à la soutenance de Quentin
	- Bien passé selon elle
      - A commencé modifs sur HDR pour pouvoir l'envoyer aux rapporteurs
      - Pense l'envoyer prochainement
    - Gérald
      - Content des soutenances de thèse
      - Doit toujours envoyer ses commentaires sur le drafts
    - Clélie
      - A enfin récupéré les 2 derniers participants dont elle avait besoin
      - Mis en place logiciel et définit table de codage pour expériences
    - Le
      - Now a Dr !
      - No plans for now
    - Matthieu
    - Olivier
      - Rien de particulier
    - Pierre-Antoine
      - Continue sur Proxy-Encryption
      - Elargit champ de recherche sur les conférences/domaines à explorer
    - Quentin
      - Now a Dr !
      - Soutenance mardi
** Semaine du <2021-01-18 Mon> au <2021-01-22 Fri>
*** Planned
**** DONE Papier : Fusionner le papier de PaPoC et le draft pour la version distribuée
- ICDCS n'a pas reporté sa deadline
  - C'est donc mort pour ça
- Du coup, vise maintenant le journal TPDS
- Doit donc remanier le format du papier en conséquence
- Idée est d'avoir un papier global avec une présentation de la version centralisée (PaPoC) puis distribuée (draft)
**** DONE Papier : Rédiger début de la section *RLS in distributed settings*
- Devait remanier de base cette section suite aux retours d'Olivier
- Le fait d'avoir fusionner les papiers accentue le besoin de la retravailler
  - Avant, présentait le mécanisme de renommage
  - Montrait la non-commutativité de l'opération de renommage avec elle-même
  - Et présentait l'intuition de comment résoudre le conflit produit par des opérations de renommage concurrentes
  - Vu qu'on présente dorénavant le mécanisme de renommage dans la section précédente, le rôle de cette section doit changer
- Définir le nouveau plan de cette sous-section
- La rédiger
**** DONE Papier : Retravailler l'introduction de la notion d'epochs
- Dans PaPoC, parlait des epochs mais ne les représentaient jamais
- C'était adapté à nos besoins vu qu'on avait juste un avant/après l'opération /rename/
- Je rentre dans plus de détails dorénavant car j'en ai besoin pour définir la relation /priority/ entre les epochs/opérations /rename/
- Les fait aussi apparaître dans mes schémas
- Retravailler l'introduction de la notion d'epochs pour rendre le papier plus cohérent
  - Autant d'un point de vue déroulé que formalisme des figures
- J'étais plutôt satisfait de ce que j'avais écrit dans le draft à ce sujet
- Voir si je peux reprendre et adapter ce contenu
**** DONE Papier : Retravailler l'introduction de la notion d'epoch tree
- Dans le draft, j'introduisais l'epoch tree afin de pouvoir expliquer comment déterminer les opérations /rename/ à inverser
- Ici, j'introduis déjà l'epoch chain dans *RLS in centralised settings*
- Pourrait "remonter" l'introduction de l'epoch tree pour pouvoir l'utiliser dans la définition de /priority/
- Pourrait même déjà avoir besoin d'en parler dans à la présentation du conflit en cas de renommages concurrents
*** Done
- Papier : Fusionner le papier de PaPoC et le draft pour la version distribuée
  - A commencé à fusionner les papiers
    - Encore quelques raccords à faire avec le nouveau template
      - Pas encore regardé/compris comment fonctionne ce qu'ils recommandent pour les sous-figures
	- Ça donne encore quelques ratés au niveau de la mise en forme, notamment des captions
      - Pas convaincu du résultat avec le package /algorithmic/ recommandé
	- Essayer /algorithmicx/
      - Reste à configurer /hyperref/
      - Le style de \paragraph diffère par rapport au template précédent
	- Retravailler les différentes sous-parties de *Results*
  - Modifications à apporter suite au changement de scope et de plan du papier
    - *Overview*
      - Repars sur le plan RLS en centralisé puis RLS en distribué
      - Ajouter une note à ce sujet dans *Proposed approach* ?
    - *RLS in centralised settings*
      - Mettre à jour le formalisme des figures avec /epochs/
    - *RLS in distributed settings*
      - Ré-écrire début de cette section
	- Plus besoin d'introduire l'opération de renommage
	- Doit juste se concentrer sur l'explication du conflit et comment on peut le résoudre en ne "choisissant" qu'une opération de renommage
      - Doit retravailler l'introduction de /revertRenameId()/
	- Doit définir les propriétés que doivent respecter /renameId()/ et /revertRenameId()/
	- Et doit donc présenter /renameId()/ et /revertRenameId()/ comme un couple de fonctions
  - Contenu supplémentaire
    - À l'heure actuelle, le papier fait 9 pages complètes
    - Manque encore
      - Abstract
      - Intro
      - 2nde version de l'algo de /renameId()/
      - Algo de /revertRenameId()/
      - Conclusion
      - Appendices
    - Si ça prend 1.5 - 2 pages, ça sera déjà pas mal
    - Le papier doit faire 12 pages
      - Jusqu'à 18 possible, mais faut payer
    - Reste donc grosso-modo 1 page de contenu de "libre"
    - Pistes possibles
      - Stratégies de renommage
	- Peut ajouter une sous-section à la fois dans *RLS in centralised settings* et dans *RLS in distributed settings* à ce sujet
	- *RLS in centralised settings*
	  - Servirait à introduire le requirement de performance
	  - Porte sur plusieurs aspects :
	    - Taille de la structure
	    - Consommation en bande-passante de l'opé de renommage
	    - Temps d'inté de l'opé de renommage sur les différents noeuds
	  - En fonction du système et des contraintes physiques/matérielles des différents noeuds, peut avoir différentes priorités
	  - Se traduit dans tous les cas par la nécessité de déclencher un renommage en fonction de la taille de la séquence
	    - Taille de la structure dépend du nombre de blocs
	    - Consommation en bande-passante de l'opé de renommage dépend du nombre de blocs
	    - Temps d'inté de l'opé de renommage dépend du nombre d'éléments à renommer
	      - Ce dernier cas de figure ne scale pas
	  - Poser la question de quand ça devient worth it ?
	    - D'un aspect gain de temps
	      - Car le renommage réduit le temps d'inté des prochaines opérations
	    - D'un aspect metadonnées
	- *RLS in distributed settings*
	  - Servirait à introduire le requirement de minimalisation des opérations concurrentes
	  - Et montrer son aspect contradictoire avec le requirement de performance
	  - Existe un état optimum sur lequel déclencher un renommage
	  - Mais doit empêcher tous les noeuds de déclencher un renommage dès qu'ils observent de manière indépendante cet état
	  - Donc nécessiter de se coordonner
	  - Mais pas trop, car veut pas retomber sur les algos de consensus qui ne scalent pas
      - Modèle de cohérence
	- Que ça soit dans le papier pour PaPoC ou le draft, je ne parle jamais du modèle de cohérence que doit respecter RLS
	- Juste dans la sous-section *System Model*, explique que RLS partage les mêmes /requirements/ que LS
	  - Livraison des suppressions après insertions correspondantes
	- Mais l'introduction de l'opération de renommage et des epochs change ça
	- RLS nécessite en plus une livraison des opérations /epoch-based/ pour ses /insert/ et /remove/
	- Faudrait que je revois pour /rename/ elle-même
	  - Dans l'implem, a mis en place une livraison causale
	  - Mais me semble que /epoch-based/ convient aussi
	  - À confirmer
	- En parler après avoir introduit ces 2 concepts, donc en fin de section *RLS in centralised settings*
      - Complexité en temps
	- Pourrait fournir une analyse de la complexité en temps des différents algos proposés
	- /renameId()/ a une complexité de /O(log(m))/
	  - Consiste à rechercher le précédesseur de l'identifiant à renommer dans une liste triée
	  - Où /m/ est le nombre d'identifiants est le nombre d'identifiants du /former state/
	  - Par contre, comment faire apparaître la taille des identifiants ?
	  - Après peut argumenter que le nombre d'éléments >> taille des identifiants
	    - Donc pas vraiment le plus important à faire apparaître dans l'analyse
	- /rename()/ a donc une complexité de /O(n * log(m))/
	  - Puisque consiste à renommer chacun des /n/ identifiants courant par rapport au /former state/ fourni composé de /m/ identifiants
	- /revertRenameId()/ a une complexité de /O(1)/
	  - Consiste à faire du /pattern matching/ pour identifier le renommage qui a été appliqué précédemment
	  - Pas besoin de boucle pour retrouver le prédecesseur si besoin
	    - Car information déjà présente /offset/
	- /revertRename()/ a donc une complexité de /O(n)/
	- Reste juste à faire apparaître la distance entre les epochs
	- Si on considère que /k/ représente le nombre de /rename/ à inverser et /l/ le nombre de /rename/ à appliquer successivement ensuite
	- On obtient que /fullRename()/ a une complexité de /O(k * n + l * (n + m))/
	  - Avec par contre les /m/ qui changent en fonction du renommage
	  - Mais devrait avoir un ordre de grandeur similaire tant que /l/ petit
	- Faudrait aussi mettre à jour les complexités de /insert()/ et /remove()/
	- /insert()/ et /remove/ ont une complexité de /O(k + l * log(m) + log(n))/
	  - Si on considère que l'insertion/suppression dans la liste triée a une complexité de /O(log(n))/
      - Preuve de la correction
	- Alors pas de la version distribuée
	- Mais la version centralisée, je pense que c'est jouable de manière "logique"
	- Nécessiterait d'introduire les propriétés que doit respecter l'algo de renommage
	  - Cohérence forte à terme
	    - Montrer le déterminisme de l'opération de renommage
	    - Et que /rename/ est commutative avec /insert/ et /remove/
	  - Conservation de l'intention
	    - Le renommage en soi n'a pas d'intention
	    - Ne doit pas interférer avec les autres opérations, /insert/ et /remove/
	    - Ça veut dire quoi ça ?
	      1. Un élément supprimé ne doit pas être "rappelé" par un renommage
	      2. Un élément inséré ne doit pas être supprimé par un renommage
	      3. Un élément ne doit pas être déplacé par un renommage
	    - Les 2 premiers points me paraissent triviaux
	      - Un noeud applique le renommage sur son état
	      - Plus précisément, le renommage va uniquement modifier l'identifiant de chaque élément
	      - Donc pas de risque de résurrection ou disparition d'un élément suite à l'application du renommage
	  - Conservation de l'ordre
	    - Montrer que si /id1 < id2/, /renameId(id1) < renameId(id2)/ dans tous les cas
	    - Ça me paraît faisable
	    - Juste que je ne sais pas écrire de preuves
	    - Donc besoin d'un coup de main
      - Implémentation alternative possible
	- Une autre façon de faire est de se baser sur le log des opés
	- Renommer consiste à repartir de l'état résultat du renommage primaire, et d'appliquer dessus les opés concurrentes
	- Remplace le facteur /n/ de la complexité de /fullRename()/ par un facteur /x/ qui correspond au nombre d'opés concurrentes
	- IMO /x << n/
	- Mais nécessite de conserver le log des opés
	  - Nécessaire par ailleurs pour la sync entre pairs ainsi que mécanisme d'anti-entropie
	  - Donc pas un si gros problème en soi
	- Mais nécessite de parcourir ce log pour en sortir les opés concurrentes
	  - Nécessite de pouvoir identifier les opés concurrentes à l'opé /rename/
	  - Donc un /vector clock/, ce qui n'est pas terrible dans un système dynamique
	- Faudrait voir la complexité en temps de cette étape là
  - Réunion
    - Faudrait trouver un meilleur nom pour la 2nde version de /renameId()/
      - Olivier proposait /renameIdDis()/
    - Concernant le contenu, vaudrait mieux améliorer/détailler le contenu existant
    - Puis ensuite, en fonction des reviews, compléter/ajouter ce qui est demandé
- Papier : Rédiger début de la section *RLS in distributed settings*
  - Ce qui a été présenté jusque là
    - Présente le mécanisme de renommage dans section *RLS in centralised settings*
    - Introduit et explique le fonctionnement de l'opération de renommage
    - Justifie et présente les transformations des opérations /insert/ et /remove/ concurrentes
  - Ce que doit faire cette sous-section
    - Illustrer que des opérations /rename/ concurrentes produisent un conflit
    - Justifier que pour assurer convergence des copies, doit le résoudre
    - Introduire approche proposée pour résoudre ce conflit
      - Absence d'intention de l'utilisateur liée aux opérations /rename/
      - Libre de résoudre le conflit comme on l'entend
      - Propose d'appliquer effectivement qu'une opération /rename/ parmi l'ensemble des opérations /rename/ concurrentes
      - Doit alors fournir aux noeuds un moyen pour qu'ils appliquent tous à terme la même opération /rename/
      - Comme ne maîtrise pas l'ordre de livraison des opérations, doit aussi fournir aux noeuds un moyen d'annuler l'effet d'une opération /rename/
	- Comme ils n'ont qu'une connaissance partielle du système et des opérations effectuées, peuvent avoir appliqué une opération /rename/ secondaire avant de recevoir la principale
	- Un mécanisme d'annulation de l'opération /rename/ permettrait de gérer ce cas de figure
- Papier : Remarques diverses
  - Titres des sections *RLS in centralised/distributed settings* pas terrible
    - Correspond plutôt à avec/sans opérations /rename/ concurrentes
    - Faudrait trouver des titres qui reflètent mieux cette idée
  - Epochs pas assez expliquées dans *RLS in centralised settings*
    - J'introduis le concept des /epochs/ brièvement dans *Dealing with concurrent updates*
    - Mais ne prend pas la peine de décrire leur structure et de les représenter
    - Pourrait déjà faire une description plus complète et présenter la relation /parent/ qui les lie
    - Permettrait alors d'introduire déjà la /GC rule 2/ dans cette section
  - L'algo de /rename()/ n'est jamais décrit
    - On présente /renameId()/ et /revertRenameId()/
    - Mais on ne présente l'algo permettant d'appliquer ces fonctions sur l'ensemble de la liste
    - Est-ce un problème ?
    - Pas évident de présenter sa complexité en temps si on ne l'a jamais montré
    - Mais en soit, algo plutôt simple
    - La difficulté réside dans comment l'implémenter de manière efficace
  - Où parler de la compression de l'opération /rename/ ?
    - Actuellement, a un paragraphe à la fin de la sous-section */rename/ operation*
    - Explique qu'on peut compresser l'opé pour n'envoyer que les éléments qui identifient de manière unique les blocs
    - Mais n'explique pas par exemple comment la décompresser
      - Repose sur une livraison causale de l'opération /rename/ dans ce cas
      - Parcours de la structure pour retrouver identifiants correspondants
      - Si identifiant manquant, a été supprimé en concurrence
      - Se trouve donc dans le log des opérations concurrentes à l'opération /rename/
    - Pourrait donc développer ce point
    - En faire une sous-section ?
      - Dans *Discussion* par exemple ?
- Papier : Retravailler l'introduction de la notion d'epochs
  - Ce qui été présenté jusque là
    - Le renommage local
    - Que d'autres opérations peuvent être générées en concurrence
    - Qu'appliquer "bêtement" ces opérations donnent un résultat incohérent
  - Ce que doit faire ces paragraphes
    - Expliquer qu'on doit détecter ces opérations concurrentes
    - Introduire la notion d'epoch
    - Expliquer l'intuition derrière
    - Expliquer comment les epochs sont construites
    - Expliquer que chaque opération est taggée avec son epoch d'origine
    - Introduire la notion de chaîne d'epochs ?
- Papier : Retravailler l'introduction de la notion d'epoch tree
  - La sous-section sur le conflit entre renommages concurrents me paraît déjà bien chargée
  - J'aurais tendance à introduire ça juste avant /priority/
- Réunion Projet Informatique
  - Points à aborder
    - Choix du thème
    - Choix du principe du jeu
    - Influences
    - Choix des fonctionnalités
    - Rappel nécessité BD
    - Choix de l'ordre de dev des fonctionnalités
    - Mise en place du repo git
    - Choix du langage de programmation
  - Groupe Lucas Seznec - Axel Thouvenin - Gaston Maimbourg
  - Groupe Thomas Champseix - Gabriel Masschelein
    - Thème
      - Post World-War, mais post-apo
    - Influences
      - Darkest Dungeon
      - Thea 2, un 4X
    - Décompose leur jeu en 2 phases principales
      - Gestion
	- Correspond au hameau de Darkest Dungeon
	- Affectation des personnages à différentes tâches/missions
	  - Correspond aux batîments de DD
	- Gère l'inventaire des personnages
      - Exploration
	- Correspond à la phase de jeu de Thea 2
	- Map hexagonale qu'on explore librement
	- Cases peuvent déclencher évènement
    - Avait réalisé un diagramme de classes plutôt complet
      - Manque pour le moment surtout des classes "intermédiaires" genre *Jeu*, *Gestion*
      - Et les méthodes de pas mal de classes
    - Avait commencé à réflechir aux fonctionnalités
      - Mais pas l'air super documenté
      - Pour certaines, avait un déroulé de la fonctionnalité
	- Clic de l'utilisateur, réponse de l'interface, nouvelle action proposée
    - Catégorisation des fonctionnalités
      - Catégorie 1
	- Chemin critique, ce que le jeu doit proposer
      - Catégorie 2
	- Sympa, ce qu'il faudrait ajouter au jeu une fois le chemin critique
      - Catégorie 3
	- Dispensable, ce qu'il faudrait ajouter
    - Librairies
      - Utilise Slick2D
	- Librairie qui met en place la boucle de jeu
	- Déjà utilisé dans le cadre d'un projet à l'IUT Nancy-Brabois
    - Leur ait demandé pour la prochaine fois
      - Une liste des fonctionnalités, catégorisées par priorité
      - Diagramme de classes MàJ en correspondance
      - Mise en place du repo git
      - Réflexion sur la DB
- Meeting
  - J'étais au labo mardi
    - Calme, paisible
  - coedit.re est down
    - Faudra enquêter là-dessus
  - Travaille sur l'article de journal pour TPDS sur *RenamableLogootSplit*
    - Ré-écrit le début de la partie sur le renommage en concurrence du draft
      - Avait eu plusieurs remarques d'Olivier sur ce qui n'était pas clair dans cette partie
      - Puis avec le changement de scope du papier, la majorité du contenu de cette sous-section était déjà présenté dans la partie précédente
      - Donc j'ai ré-écrit ça pour développer le problème des renommages concurrents et l'approche proposée
    - Effectué des modifs dans le contenu de PaPoC
      - Pour des raisons de place, avait survolé certains points comme les epochs
      - J'ai retravaillé ces parties pour rendre les explications et les figures plus claires
    - Mais doit modifier les sections du draft en conséquence
      - Pour éviter les répétitions et pour garder une cohérence tout au long du papier
    - Reste à voir taille du papier une fois que j'aurai fini cette passe sur le contenu et ajouter ce qu'il manque
      - Abstract, intro, conclusion, annexes
      - Voir si peut ajouter du contenu
  - A vu que Kleppmann et al. ont un draft sur *A highly-available move operation for replicated trees and distributed filesystems*
    - La suite de leurs travaux publiés à PaPoC
    - A l'air de viser TPDS
    - Faudra que j'y jette un oeil
  - Réunion
    - François
      - Fini ses corrections
      - Réunion avec Arial (?) avec Gérald et Victorien
	- Problématique d'édition de fichiers de configuration Scada (?)
      - Reçu 2nd rapport positif pour Béatrice
	- Devrait soutenir le 3 février
    - Alexandre
      - Occupé avec le projet de compil
      - Continue recherche de librairies pour récupérer données de Wikipedia
    - Anis
      - A effectué corrections chapitre 4 et 5 suggérées par Olivier
      - En train de voir pour faire valider les formations pro de la thèse grâce à Open
    - Claudia
      - A été aux voeux de Bruno
	- Étudiants doivent pas hésiter à aller au labo
	- Observe les flux, si dépasse seuil, mettront en place contrôle
      - Sera Chair pour workshop à ECSCW'21
	- Ont prévu une conf' hybride
	- À voir comment la situation évolue
      - Pas mal occupée avec les cours
    - Gérald
      - Réunion avec Matthieu et Olivier en début de semaine
      - Réunion avec Arial (?)
    - Le
      - Need to prepare 5-10 pages in French of the manuscript
      - Took his introduction, translated it using Google Trad
      - Asked to Quentin and Matthieu to help
    - Matthieu
    - Olivier
      - Réunion avec Matthieu et Gérald
      - Réunion avec JP
	- Travaille actuellement sur un poster
    - Pierre-Antoine
      - Continue recherches sur Proxy Encryption
      - Implémentation en TypeScript
	- S'inspire de son implem précédente en Python
      - Doit envoyer lien vers biblio avec résumés et annotations
    - Quentin
      - Effectue corrections pour le manuscrit
	- A eu une rapportrice au tel lundi pour voir tout ça
      - Attend des nouvelles de l'école doctorale pour le dépôt de son manuscrit
** Semaine du <2021-01-25 Mon> au <2021-01-29 Fri>
*** Planned
**** DONE Papier : Ajouter exemple pour l'utilisation de /priority/
**** DONE Papier : Réserver salle pour réunion <2021-01-29 Fri> à 14h
**** DONE Papier : Ajouter la relation /priority/ et l'epoch courante à la figure de *Breaking tie between concurrent rename operations*
- Permettrait de compléter cette figure et d'introduire le formalisme pour /priority/ et l'epoch courante
- Simplifiera la compréhension des figures pour la GC
**** DONE Papier : Envoyer cette version à Olivier et Gérald pour préparer la réunion de <2021-01-29 Fri>
*** Done
- Réunion Projet Informatique
  - Points à aborder
    - Choix du thème
  ...
  - Groupe Lucas Seznec - Axel Thouvenin - Gaston Maimbourg
    ...
    - Principe du jeu
      ...
      - Combat de boss
      - Menu sélection
	- Permet de refarmer un boss précédent
	- Passer au boss suivant
    - Influence
      - Pokemon
      - Monster Hunter
      - Sword & Sandals
    - Absence d'évènements
    - Fonctionnalité
      - PVP en ligne :o
      - Mode co-op
      - Inventaire
      - Sauvegarde progression personnelle
	- Mon personnage, son niveau, son build
	- L'état de la partie, quels boss vaincus et leur niveau
    - Choix du langage
      - Java
    - Warning
      - Arbres de compétences
- Ajouter exemple pour l'utilisation de /priority/
  - Ajout d'une figure représentant les /epoch trees/ de A et de B au cours de l'exécution du scénario utilisé dans le papier
  - Ajout d'un paragraphe expliquant comment en utilisant /priority/ on détermine que /eA1/ et /eB2/ sont prioritaires par rapport à /e0/
  - Ajout d'un paragraphe expliquant comment en utilisant /priority/ on détermine que /eB2/ est prioritaire par rapport à /eA1/
  - Ajout d'une précision que comme /eB2/ était déjà l'epoch courante de B, celui-ci n'a rien à faire à la réception de /eA1/
  - Mais qu'à l'inverse, A doit renommer son état de /eA1/ à /eB2/ lui
    - Ce qui m'offre la transition pour la prochaine section
- Papier : Retravailler section *Applying primary rename operations*
  - Ce qui a été présenté jusque là
    - Appliquer des renommages concurrents naïvement provoque une divergence
    - Peut résoudre ce problème en se contenant de "jouer" qu'un renommage de l'ensemble des renommages concurrents
    - Pour déterminer quel renommage choisir, "suffit" de définir une relation /priority/ définissant un ordre total sur les epochs
    - Les noeuds ayant appliqués un ou des renommages concurrents doivent renommer leur état vers la nouvelle epoch courante
  - Ce que doit faire cette sous-section
    - Montrer qu'appliquer directement le /primary/ renommage ne mène pas au bon état
      - L'état final de A dans l'exemple illustre cela
    - Les noeuds ayant appliqués des renommages concurrents doivent revenir à l'epoch sur lequel le renommage est définit
    - Pour ça, les noeuds doivent potentiellement annuler des renommages
    - Introduire /revertRenameId()/
      - But
	- Annuler l'effet d'un renommage
	- Retrouver l'id correspondant à chaque id qui a été renommé
	- Fournir un id à tous les ids qui ont été générés après ce renommage
      - Propriétés à respecter
	- Déterminisme
	- Réciprocité avec /renameId()/
	- Préservation de l'ordre entre ids
      - Algo de /revertRenameId()/
      - Algo de /renameId()/ mis à jour
    - Pour déterminer les renommages à annuler, utilise l'arbre des epochs
      - En calculant le Lowest Common Ancestor entre l'epoch courante et l'epoch visée
      - Le chemin inverse entre le LCA et l'epoch courante correspond aux renommages concurrents appliqués qu'il faut annuler
    - Puis reste simplement à appliquer les renommages entre le LCA et l'epoch visée
    - Illustrer cela à l'aide d'un exemple
      - Peut reprendre l'exemple fil rouge du papier, en faisant converger ce coup-ci
      - Montrer qu'appliquer /rename to eB2/ sur A consiste à /revert to e0/ puis /rename to eB2/
      - Montrer qu'appliquer /rename to eA1/ sur B consiste à rien faire
- Meeting
  - J'étais au labo mardi
    - Bien sympa
    - Mais problèmes d'internet j'ai l'impression
    - Ce qui m'a été confirmé par le mail de ce matin
  - Continue sur l'article sur RLS
    - J'ai fini en début de semaine mes modifs par rapport à la notion d'epochs
      - Introduction plus tôt
      - Introduction plus tôt de l'arbre des epochs, la structure que maintienne les noeuds pour suivre où ils en sont dans les renommages
    - J'ai envoyé cette version "qui se tient" à Gérald et Olivier pour qu'on fasse une passe sur mes modifs au cours de notre prochaine réunion
      - Prévue aujourd'hui mais décalée à lundi
    - Faut maintenant que je vois pour retravailler ma sous-section portant sur l'algo d'inversion du renommage
  - Mais avant de me lancer dans ces modifs, me suis mis sur mes corrections qui traînent depuis 2-3 semaines
    - DMs d'Algo, Projet de BD2
  - Assisté à la fin de la défense de Théo Biasutto-Lervat
    - Il est Dr. depuis 13h
  - Réunion
    - François
      - Soutenance de Béatrice <2021-01-27 Wed> à 14h
      - Entretien annuel avec Jean-Yves Marion
	- Evaluation HCERES repoussé d'1 an
      - Réunion des permanents
	- Pour décider prochaines directions de l'équipe
	- Samir cherche à partir à Paris
	- Pascal Urso a fait un échange de poste avec quelqu'un de Nice
      - Réunion responsables d'équipe
	- Tout le monde est moins productif
	- Moins de dynamisme
    - Alexandre
      - Soutenance avec apprentis TN
      - Réunion avec Claudia
      - Travaille sur l'import de dump de BD de wikipedia
	- Y en a pour 60 jours avec son utilitaire
	- À la recherche d'autres utilitaires du coup
    - Anis
      - Accaparé par des corrections et problèmes techniques
    - Claudia
      - A fini les cours de réplication
      - Finit de préparer l'examen
      - Utilise Arche pour son partiel
      - A eu réunion avec Thomas Lambert avec François
	- Candidat Maitre Conf dans notre équipe pour la FST
	- Postule aussi sur un poste Inria
	- Travaille sur des algorithmes de placement de répliques (?)
    - Olivier
      - Plutôt orienté cours en ce moment
      - Avec JP et Thibault, a envoyé poster à conf réseau
      - A vu passer un article sur un protocole à base d'epochs
    - Pierre-Antoine
      - Continue son implémentation
      - Essaie de faire le rapprochement entre les applications collaboratives et les problématiques de contrôle d'accès
	- Tracer des liens entre les différents thèmes
      - A repris contact avec Jérôme Blanchard pour bosser sur Fair & Smart
    - Quentin
      - Réunion avec François
	- À propos de la suite
	- Réfléchit à quelques possibilités de post-doc
** Semaine du <2021-02-01 Mon> au <2021-02-05 Fri>
*** Planned
**** DONE Algo : Noter les DM3
*** Done
- Réunion Projet Informatique
  - Groupe Lucas Seznec - Axel Thouvenin - Gaston Maimbourg
    - Ce qui était demandé
      - Une liste des fonctionnalités, catégorisées par priorité
      - Diagramme de classes MàJ en correspondance
      - Ajout des fichiers sur le repo sur GitHub
      - Début de réflexion sur la BD
    - Réunion
      - Remarques sur fonctionnalités
	- Pas sûr de comprendre comment les évènements s'intégrent dans votre jeu d'après votre diagramme
	- Beaucoup de fonctionnalités en priorité forte
	  - Comment vous voyez cette catégorie ?
	  - Est-ce qu'il s'agit de la v1 ou vous avez en tête une version allégée en prototype ?
      - Pour la prochaine fois
	- Mettre le diagramme de fonctionnalités sur Git
	- Réalisation/Complétion du diagramme de classes
	  - Avec les fonctionnalités prioritaires au moins (Combat, Personnage, Boss, Carte)
	- Commencer à réfléchir au schéma relationnel de la BD
      - Prochaine réunion <2021-02-10 Wed> à 11h
  - Groupe Thomas Champseix - Gabriel Masschelein
    - Ce qui était demandé
      - Une liste des fonctionnalités, catégorisées par priorité
      - Diagramme de classes MàJ en correspondance
    ...
    - Réunion
      - Pas de but
      - Priorité
	- 1ère version : Exploration
	  - Personnage, Carte, Evènements, Combat (résolution automatique)
	- 2nd version : Inventaire codé en dur
	  - Equipement, consommables, mais pas de gestion
	- 3ème version : Gestion
	  - Ajout de missions, gestion des missions, gestion de l'inventaire
      - Pour la prochaine fois
	- Mettre le diagramme des fonctionnalités sur Git
	- Mettre le diagramme de classes sur Git
	- Mettre feuille de route sur Git
	- Mettre en place le squelette de la 1ère version
	  - Classes, les différentes méthodes
      - Prochaine réunion <2021-02-11 Thu> à 14h
- Réunion papier
  - Modifications
    - Introduction des epochs et de l'epoch chain dans 4.2
    - Ré-écriture de la 5.1
    - Introduction de l'epoch tree dans la 5.2 et ré-écriture
      - Avant, le présentais dans la section 5.3 pour introduire la notion de Lowest Common Ancestor entre 2 epochs
      - Là l'utilise pour définir notre relation /priority/ du papier
    - Disparition/Fusion de 5.3 avec la sous-section suivante
  - À faire
    - Re-travailler 5.3
      - Poser sous la forme de propriétés les contraintes que doit respecter /revertRenameId()/ ?
      - Dans le draft, la version de /renameId()/ présentée prenait déjà en compte /revertRenameId()/
      - Là on présente la version simple de /renameId()/ en 4.2
      - Faut introduire la nouvelle version
    - Conclusion
    - Intro
    - Abstract
    - Correction de la réciprocité de /renameId() v2/ avec /revertRenameId()/
  - Pistes
    - Où parler de la GC ?
      - Actuellement, aborde le sujet à la fin de la partie sur le mode distribué
      - Rule 2 est déjà valide/nécessaire pour la GC des formers states dans le mode centralisé
      - Pourrait déjà la présenter à la fin de la partie sur le mode centralisé
      - Et illustrer pourquoi elle n'est plus suffisante et la compléter dans la 5.4
    - Où parler de la compression de l'opération /rename/ ?
      - Actuellement, a un paragraphe à la fin de la sous-section */rename/ operation*
      - Explique qu'on peut compresser l'opé pour n'envoyer que les éléments qui identifient de manière unique les blocs
      - Mais n'explique pas par exemple comment la décompresser
	...
	- Dans *Discussion* par exemple ?
  - Réunion
    - Gérald lit ça cette semaine
    - Olivier lit dès que possible
      - Difficile cette semaine
      - Envoyer nouvelle version vendredi/lundi
    - Pour l'instant, remplir
      - Ajouter contenu sans tenir compte de limite de pages
      - On verra ensuite pour faire le ménage
    - Pour GC
      - Garder la sous-section à la fin de la partie distribuée
      - Peut faire référence en avant à la fin de la partie centralisée pour pointer vers règle pertinente
    - Plan
      - Finir rework de la section 5
      - Ajouter conclusion, intro et abstract
      - Puis compléter les TODO? restants
      - On voit au fur et à mesure que j'obtiens les retours pour les intégrer
- Meeting
  - Rien de particulier cette semaine
  - Une réunion lundi avec Olivier et Gérald pour faire le point rapidement
    - Et discuter du planning à court terme
    - Vise l'envoi fin février
    - Ça continue son bonhomme de chemin
  - Occupé par des corrections le reste de la semaine
  - Réunion
    - François
      - Béatrice a soutenu et obtenu le titre de Docteur mercredi aprem
      - Réunion avec Areal
	- Projet de redévelopper tout leur système de fichiers de config pour faire de l'édition collaborative
      - Bientôt responsable de la Commission Info pour l'école doctorale
      - Réunion avec ??? pour de la communication autour de l'équipe Coast
      - Présentation la semaine pro par Thomas LAMBERT
	- Candidat comme Mcf ou CR dans l'équipe
      - A reçu attestations de réussite de Quentin et Le
    - Abir
      - Reprise des cours en présentiel
      - Rédaction du rapport
	- Rédige en anglais
	- Blocage en français, a décidé de switch en anglais
	- A permis de débloquer
	- À voir pour la dérogation
    - Alexandre
      - Continue de regarder les technos pour pré-traiter les dumps de Wikipedia
	- A fait une short-list
      - Réunion avec Claudia cet aprem pour en discuter
    - Anis
      - Réunion avec François et Olivier
	- Parler de la dernière solution de composition de services
	- Part sur 2 articles
	  - Un sur la sélection de services
	  - L'autre sur la composition
    - Claudia
      - Examen en distanciel du module de réplication de données sur Arche
	- Bien passé
      - Réunion commission d'évaluation inria
	- Équipe devrait être évalué en octobre avec nouveau format
	- Mais nouveau format non-validé par HCERES
	- Si nouveau format pas approuvé dans les prochaines semaines, sera évalué avec l'ancien
      - Réunion avec Pierre-Antoine
    - Khalid
      - Rien de spé
    - Moi
    - Pierre-Antoine
      - Préparé réunion avec Claudia et Olivier
      - A présenté les pistes explorées
      - Quelques scénarios manquants
      - Explore capacity base access control
    - Quentin
      - S'est intéressé à l'après-thèse
      - A fait un research statement
      - Discussion avec François sur un des travaux non-publié
	- Interactions dans les écoles primaires
	- Discuter pour voir quelles conférences viser
	- Vise CSCW pour le moment
** Semaine du <2021-02-08 Mon> au <2021-02-12 Fri>
*** Planned
**** DONE Papier : Retravailler section *Applying primary rename operations*
- Ayant fait pas mal de modifs sur les sections précédentes, cette section ne doit plus être vraiment raccord
- Le format ayant aussi changé, j'ai p-e des éléments à développer dans cette section
- Refaire une passe dessus et la retravailler en conséquence
**** DONE Papier : Retravailler section *Garbage collection of former states*
- Pareil, voir si cette section est toujours raccord
- En profiter pour mettre à jour les figures
  - En rouge l'epoch prioritaire
- Expliquer aussi la représentation des epochs causalement stables
**** DONE Papier : Envoyer nouvelle version à Gérald et Olivier
- La nouvelle version de la section *Applying primary rename operations* me paraît bonne
- Celle de *Garbage collection of former states* moins
  - Mais je vois moins comment l'améliorer
- Dans le doute, envoyer la version actuelle
- Ça fera une base plus récente pour qu'ils fassent leurs retours
**** DONE BD2 : Noter les projets
- Peut décomposer le projet en 2 notes
  - Conception et SQL
    - Juste évaluer si la schéma relationnel proposée est correct, permet de répondre aux requêtes indiquées
    - Et vérifier que les requêtes sont valides
  - Mise en pratique
    - Évaluer que le script SQL construit une BDD correspondante au schéma relationnel
    - Évaluer le code et les interactions avec la BDD
**** DONE Papier : Ajouter conclusion
- La partie contenu du papier a l'air globalement complète
  - Reste des points à développer, mais le gros est déjà présent
  - Ne devrait plus trop évoluer
- Peut donc passer aux différents parties restantes
- Commencer par la conclusion me paraît le plus indiqué
- Avait commencé à rédiger celle du draft
- Doit donc voir pour la reprendre, l'adapter et la développer
**** DONE Papier : Compléter sous-section *Proposed approach*
- Ajouter quelques lignes pour indiquer que RLS est un Op-based CRDT
**** DONE BD2 : Envoyer les notes à Polytech
- J'ai envoyé les notes aux étudiants
- J'attend de voir 1-2 jours si certains se manifestent pour discuter de leur note et si ça me conduit à ré-évaluer certains projets
- Envoyer les notes à Polytech <2021-02-11 Thu> grand max
**** DONE Papier : Ajouter sous-section *Definition of rename operation*
- Ajouter quelques lignes présentant le but de l'opération et les propriétés qu'elle doit respecter pour être valide
*** Done
- BD2 : Noter les projets
  - A fini de les corriger
  - Reste plus qu'à donner une note à tout ça
- Papier : Retravailler section *Applying primary rename operations*
  - Rework de l'intro de cette sous-section pour justifier et introduire l'inversion du renommage
  - Présente l'objectif de /revertRenameId()/
  - Illustre son utilisation
  - Insiste sur le fait que /renameId()/ et /revertRenameId()/ doivent être des fonctions inverses
  - Mentionne la mise à jour de /renameId()/ pour prendre en compte les identifiants ayant été insérés causalement après ce renommage
  - Ajoute la nouvelle version de /renameId()/ en annexe
- Papier : Retravailler section *Garbage collection of former states*
  - Je ne vois pas de grosse modifs à faire en vrai
  - Ajout d'une phrase pour insister sur le fait que la stabilité causale seule n'est pas suffisante pour déterminer quelles opérations sont GC-ables
  - Complétion de la figure pour faire apparaître quelles epochs sont GC-ables et par quelle règle
    - J'ai tenté en dessinant une croix dessus
    - Résultat pas ouf
    - Pars plutôt sur une représentation dotted ou dashed en couleur
    - Pas ouf non plus, mais j'ai pas mieux comme idée
  - Modification du texte pour expliquer un peu mieux les figures
    - Formalisme des epochs causalement stables
    - Formalisme des epochs GC-ables
- Papier : Ajouter conclusion
  - Qu'est-ce qu'on a fait ?
    - Présenter un nouveau CRDT : RenamableLogootSplit
  - En quoi il diffère de l'existant ?
    - Embarque un mécanisme de renommage
  - Pourquoi ?
    - Permet de réduire la quantité de métadonnées consommée par la structure
  - Ça consiste en quoi ?
    - Un nouveau type d'opération, les opérations de renommage
    - Opérations systèmes déclenchées automatiquement par les noeuds lorsque les conditions sont remplies
    - Mais comme opé de renommage pas commutative avec le reste (insert, remove, rename), des conflits se produisent en cas d'opérations concurrentes
    - RLS applique des techniques OT pour résoudre ces conflits
  - Qu'est-ce qu'on a fait ensuite ?
    - Procédé à une validation expérimentale de RLS et avons validé sa correction et évalué ses performances
    - Constaté que permet de diviser par centaines les métadonnées de la structure à terme
    - Mais que peut provoquer une inflation des métadonnées en attendant, notamment en cas d'opérations de renommage concurrentes
  - Ok, c'est quoi la suite ?
    - Déterminer quand déclencher un renommage
      - 2 contraintes à gérer : garder temps d'intégration en-dessous d'un seuil et minimiser le nombre d'opérations de renommmage
    - Preuve formelle de la correction des algos de RLS
    - Étudier si d'autres types de données répliqués gagnerait à combiner les approches CRDT et OT pour leur design
- Papier : Ajouter sous-section *Definition of rename operation*
  - Principe et but de l'opération
    - Doit réattribuer de nouveaux identifiants à l'ensemble des éléments de la séquence
    - Ne doit en aucun cas altérer le contenu de la séquence
    - Comme les identifiants sont des métadonnées utilisées par la structure de données uniquement pour la résolution de conflits
    - S'agit d'une opération système réalisée en coulisses, sans intervention de l'utilisateur
  - Propriétés que l'opération doit respecter
    - Déterminisme
    - Préservation de l'intention utilisateur
      - Ne peut pas ignorer ou altérer une opération de l'utilisateur, /insert/ ou /remove/, à cause d'une opération /rename/
    - Préservation de la relation d'ordre existante
    - Commutativité avec autres opérations
      - Puisque les noeuds peuvent recevoir les opérations dans des ordres différents
  - Commutativité difficile à obtenir
    - Puisque l'opération /rename/ modifie les identifiants de la structure
    - Mais que les autres opérations reposent sur les identifiants pour indiquer la position où insérer un élément ou quel élément supprimer
    - /rename/ est par définition incompatible avec les opérations /insert/ et /remove/
    - De la même manière, deux opérations /rename/ peuvent attribuer des identifiants différents à un même élément
    - /rename/ est donc aussi non-commutative avec elle-même
    - Il est donc nécessaire de mettre en place des mécanismes de résolution de conflits en cas d'opération concurrente à l'opération /rename/
    - Pour des raisons de simplicité
      - Présentons dans un 1er temps l'opération /rename/ avec pour hypothèse l'impossibilité d'émettre des opérations /rename/ concurrentes
	- Correspond à des systèmes où les noeuds se coordonnent pour déclencher un renommage
	- Nous permettra de présenter le fonctionnement de l'opération /rename/ et comment la rendre commutative avec les opérations /insert/ et /remove/
      - Puis dans un 2nd temps l'opération /rename/ dans le cadre de systèmes permettant les opérations /rename/ concurrentes
	- Donc sans aucune coordination entre noeuds pour déclencher un renommage
	- Nous permettra de présenter les techniques à mettre en oeuvre pour rendre l'opération /rename/ commutative avec elle-même
- CRDT : Lire *A higly-available move operation for replicated trees and distributed filesystems*
  - Propose un Tree CRDT permettant 3 types d'opérations
    - insert, remove et move
  - En fait, exprime toutes ces opérations sous la forme de move
    - /move t p m c/
    - Où /t/ est un timestamp, /c/ l'identifiant du noeud déplacé, /m/ ses métadonnées (nom du fichier par ex.) et /p/ son nouveau parent une fois le move effectué
    - insert est géré en créant /c/ si celui-ci n'est pas déjà présent dans l'arbre
    - remove est géré en définissant un /p/ arbitrairement qui correspond à la poubelle
  - Sauf que deux moves concurrents peuvent introduire différents problèmes
    - Divergence en fonction de l'ordre d'application des moves déjà
    - Création de cycle si le 1er move fait de /c/ un ancêtre de /p/
  - Pour éviter tout ça, utilise un log des opérations /move/
  - Les opérations y sont ordonnées à l'aide de leur timestamp
  - Les opérations de ce logs sont augmentées avec le couple (oldParent, oldMetadata) calculé lors de l'application du move
  - À la réception d'une nouvelle opération, undo toutes les opérations avec un timestamp supérieur
  - Applique l'opération
  - Puis redo le reste des opés
    - Potentiellement met à jour l'opération enrichie du coup si je pige bien
  - Effectue juste une vérification à base de "Est c ancêtre de p" avant d'appliquer un move pour empêcher l'introduction de cycle
  - Comme ordre effectif d'application identiques pour tous les noeuds, converge
  - Remarques
    - Le mécanisme de résolution de conflits se base finalement uniquement sur le timestamp
      - Entre 2 opérations, c'est l'ordre de leur timestamp qui va déterminer le résultat
      - Par exemple si move et remove concurrents, 2 résultats possibles
	- Soit le move est appliqué en 1er, puis le remove
	- Dans ce cas, le noeud est supprimé
	- Soit le remove est appliqué en 1er, puis le move
	- Dans ce cas, le noeud "ré-apparaît" à sa nouvelle position
      - D'un point de vue expérience utilisateur, est-ce que ça a du sens ?
- Réunion Projet Informatique
  - Notes pré-réunion
    - S'agit de la dernière réunion de la période
      - Ensuite les étudiants seront en entreprises pour 3-4 semaines
      - Reviennent donc pour la semaine du <2021-03-15 Mon>
      - À confirmer
      - C'est bien ça
    - À faire pour la prochaine fois
      - Compléter diagramme de classes si besoin et l'uploader sur Git
      - Définir les objectifs d'une 1ère version du jeu
	- Sélectionner et lister les différentes fonctionnalités du jeu que vous souhaitez implémenter pour un 1er prototype
      - Décider de la répartition des tâches
	- À partir du diagramme de classes et des dépendances qu'il met en évidence
	- Vous répartir le développement des composants du jeu
      - Démarrer développement du jeu
	- Un de vous créé le projet Java
	  - Un simple Hello World au début
	- Le push sur le repo Git
	- Les autres le récupère et vérifie qu'ils arrivent à le lancer
	- À partir de là, chacun commence à réaliser ses tâches
  - Groupe Lucas Seznec - Axel Thouvenin - Gaston Maimbourg
    - Ce qui était demandé
      - Mettre le diagramme de fonctionnalités sur Git
	- Ça a été fait
	- Pas l'impression qu'il ait été modifié depuis la dernière réunion
	- On retrouve les principales fonctionnalités
      - Réalisation/Complétion du diagramme de classes avec les fonctionnalités prioritaires (Combat, Personnage, Boss, Carte)
      - Commencer à réfléchir au schéma relationnel de la BD
    - Réunion
      - Remarques sur *Evenement*
	- Manque l'effet des évènements
	  - Je leur ai suggéré de faire une hierarchie de classes
	  - Avec des évènements qui déclenchent combats, des évènements qui appliquent buff/debuf
      - *Personnage*
	- Juste un recueil de statistiques
	- Aucune méthode à soi
      - *Joueur*
	- Correspond à l'avatar du personnage
	- Pour le moment, des méthodes pour se créer/charger/supprimer
	- A indiqué que c'était un peu étrange
	  - Un peu le paradoxe de l'oeuf et la poule
	- Ont remonté ces méthodes dans *Partie*
	- Ont commencé à ajouter des méthodes au *Joueur*
      - On avait commencé à parler de la BD, mais j'avais préféré mettre ça de côté en attendant de voir le reste du diagramme de classe
      - Vu qu'on a dû mettre fin à la réunion vu l'heure qui avançait, on est pas revenu là dessus
      - Faudra en rediscuter avec eux la prochaine fois
      - Note : Gaston est globalement celui qui anime le plus les réunions
	- À voir si ça se confirme lors des prochaines
    - À faire d'ici la fin de la semaine
      - Ajouter les associations manquantes dans le diagramme
	- Les associations de base, mais aussi celle d'héritage
      - Ajouter les différentes méthodes de chaque classe
      - Upload du diagramme de classes sur Git
	- J'ai dit impérativement d'ici la fin de semaine
    - C'est quand la prochaine réunion ?
      - Je leur ai dit que je les recontactais à leur retour le <2021-03-15 Mon>
      - Histoire de convenir d'une date pour un prochain point
  - Groupe Thomas Champseix - Gabriel Masschelein
    - M'ont envoyé un mail à propos d'utiliser Python plutôt que Java comme techno
      - Ils rencontrent des problèmes pour configurer Java semblerait
      - Voir avec quels sont ces problèmes
      - Mais ne voit pas d'objection à ce qu'ils utilisent Python perso
    - Ce qui était demandé
      - DONE : Mettre le diagramme des fonctionnalités sur Git
	- Fichier excel disponible sur le repo qui liste les différentes fonctionnalités avec un score de priorité
	- Le score ne correspond pas forcément aux versions que j'ai noté lors de la dernière réunion
      - TODO : Mettre le diagramme de classes sur Git
      - TODO : Mettre feuille de route sur Git
      - IN-PROGRESS : Mettre en place le squelette de la 1ère version
	- Seulement un main() pour le moment
	- Ont ajouté la librairie Slick2D à leur projet
    - Réunion
      - À faire d'ici la fin de la semaine
	- Compléter diagramme de classes si besoin et l'uploader sur Git
	- Décider de la répartition des tâches
      - Versions
	- Un peu mis à jour la feuille route
	- 1ère version : exploration
	  - On dirige une équipe de personnage (mais sans que Personnage existe réellement) sur une carte et on déclenche des events
	- 2nde version : personnage et combat et objets
	  - Possibilité de déclencher un combat lors de l'exploration, les personnages de l'équipe y prennent part
	  - Les personnages ont un inventaire qui contient leur équipement
	- 3ème version : gestion
	  - Ce qui était noté précédemment
      - Les maps hexagonales, finalement c'est plus compliqué qu'on dirait
	- Faut trouver des ressources sur comment représenter facilement une map hexagonale
    - À faire d'ici la fin de semaine
      - Mettre la dernière du diagramme au classe au format xml et png
      - Nettoyer le repo
	- Virer le code la version java
	- Diagramme de classe au format de PDF
	- Fichier notes.txt
    - À la rentrée
      - Me recontacter pour fixer une réunion
      - Finir le diagramme de classes si besoin par rapport aux derniers points qu'on a abordé
      - Démarrer l'implémentation du jeu en Python
- Meeting
  - Principalement bossé sur le papier
    - Complété en début de semaine mes modifs de la partie contribution
      - Gestion des renommages concurrents et le mécanisme de GC
    - J'ai envoyé cette version à Gérald et Olivier lundi
    - Ajout de la conclusion
    - Ajout de quelques modifs sur la section *Overview*
      - Histoire d'être un peu plus exhaustif et précis sur ce qu'est RenamableLogootSplit
      - Ajoute une nouvelle sous-section pour présenter les propriétés que doit respecter l'opération /rename/
  - Commencé à lire *A higly-available move operation for replicated trees and distributed filesystems*
    - Intéressant
    - Proposent une structure d'arbre répliquée permettant d'ajouter, supprimer et déplacer un noeud
    - Finalement redéfinit toutes les modifications sous forme de /move/
    - Pour gérer les opérations concurrentes, se base sur un log des opés
    - Ordonnent les opérations de manière totale à l'aide de timestamp
    - Puis l'état de l'arbre est calculé en jouant les opérations dans l'ordre
    - Donc si reçoit une opération antérieure
      - Undo les opérations "suivantes" déjà appliquées
      - Applique l'opération reçue
      - Puis ré-applique les opérations "suivantes"
    - Vérifie avant d'appliquer une opé si elle n'introduirait pas un cycle
    - Paraît straightforward et correct
    - Par contre, la gestion de conflits me paraît obscure pour un utilisateur
      - Seul le timestamp compte, pas la sémantique des opérations
      - Donc un /delete/ peut perdre contre un /move/ ou gagner en fonction des cas
    - J'ai pas encore lu la partie évaluation, mais ont mentionné avoir des bonnes perfs
      - À voir le scénario évalué
      - Mais à voir comment ils font pour ne pas faire exploser le temps d'intégration en cas de "retard" d'une opération
      - Quid des métadonnées ?
    - À comparer à des algos d'OT
      - Mais est-ce que le principe est différent ou juste la formalisation l'est ?
    - À suivre
  - Réunion
    - François
      - Rien de spé
      - Soutenance de Vic probablement reportée
      - Travaille sur le rapport d'activité
      - Réunion avec Quentin à propos de sa recherche de post-doc
      - Réunion avec Areal
      - Pas dispo <2021-02-19 Fri> prochain
    - Abir
      - Poursuit la rédaction de sa thèse
    - Alexandre
      - Travaille sur un parser pour transformer un dump Wikipédia en BD relationnel
      - Presque fini
      - Doit juste en discuter avec Gérald
    - Anis
      - Réunion avec François et Olivier
      - En train de terminer les corrections du chap 5 de sa thèse pour en faire un article
      - Travaille sur un algo génétique (?)
	- Trop lent, à optimiser
    - Claudia
      - Chair de workshop et master classes à ECSCW 2021
      - Ont fait la review cette semaine
	- A l'air de mettre l'accent sur la résilience des plateformes de CSCW en situation de crise
      - A travaillé sur le rapport d'activité
	- Quelques difficultés avec la plateforme
      - Contactée par Justine Gallet pour faire un portrait-vidéo pour le 8 mars
    - Clélie
      - Analyse ses données d'expériences
      - Quelques résultats rigolos
      - Commence à rédiger là-dessus
      - Commence aussi à préparer la 2nde XP
    - Olivier
      - Réunion avec Anis
      - Commencé à lire l'article de Matthieu
    - Pierre-Antoine
      - Continue de lire sur Capability-based Access Controls
** Semaine du <2021-02-15 Mon> au <2021-02-19 Fri>
*** Planned
**** DONE Papier : Rédiger paragraphe sur le modèle de livraison des opérations dans sous-section *Dealing with concurrent updates*
**** DONE Papier : Ajouter une phrase indiquant qu'on a utilisé que des opérations /insert/ dans notre évaluation
- Vérifier que la complexité est la même dans le papier LogootSplit
- Justifier grâce à ça
**** DONE Papier : Rédiger abstract
**** DONE Papier : Revamp algo de /renameId()/ dans la section *RLS in centralised settings*
- Pour le moment, j'ai copy/paste l'algo de PaPoC
- Mais j'avais retravaillé cet algo depuis pour le décomposer en plusieurs cas
- On trouve la nouvelle  version dans le draft
- Mais elle inclut aussi des changements pour rendre réciproques /renameId()/ et /revertRenameId()/
- Faire un mix entre ces 2 versions
  - Décomposé en plusieurs cas
  - Mais sans les ajouts requis par la version distribuée
**** DONE Papier : Compléter bibliographie
- Dans l'intro de l'article pour PaPoC, je mentionnais en Local-First Software et l'édition collaborative comme exemple d'utilisation des CRDTs
  - D'ailleurs pourrait citer les travaux de Claudia et Mehdi en plus pour l'édition collaborative
- Mais existe d'autres exemples d'utilisation des CRDTs que je pourrais citer dans la nouvelle intro
- Je pense notamment à Riak, Redis, Anna et AntidoteDB
- Faudrait que je trouve des références pour
- Dans le doute, ça me sera utile tôt ou tard dans ma rédaction
*** Done
- Papier : Rédiger paragraphe sur le modèle de livraison des opérations dans sous-section *Dealing with concurrent updates*
  - Finalement, 1 phrase me paraît suffisante
- Papier : Compléter la bibliographie
  - [X] Pour Riak
    - *Riak core: building distributed applications without shared state*
      - Par Rusty Klophaus en 2010
      - Dispo ici : https://www.researchgate.net/publication/261850079_Riak_Core_building_distributed_applications_without_shared_state
    - Faudrait juste que j'arrive à accéder à l'article pour vérifier
    - Sinon il y a l'article suivant qui a l'air d'être celui généralement cité
    - *Riak DT Map: A Composable,Convergent Replicated Dictionary*
      - Par Russel Brown, Sean Cribbs, Sam Elliott et Christopher Meiklejohn
      - Présenté à PaPEC'14
      - Dispo ici : https://pages.lip6.fr/syncfree/attachments/article/46/WP3-report.pdf#page=31
  - De ce que je comprends, existe aussi SwiftCloud qui est construit au-dessus de Riak
  - [X] Pour SwiftCloud
    - Plusieurs références disponibles
    - *Key-CRDT Stores*
      - Par Valter Balegas en 2012
      - Dispo ici : https://run.unl.pt/bitstream/10362/7802/1/Sousa_2012.pdf
      - Du mal à piger ce que c'est comme document
      - Un article de blog de Murat Buffalo mentionne que c'est une /master thesis/
    ...
    - *SwiftCloud: Fault-Tolerant Geo-Replication Integrated all the Way to the Client Machine*
      - Par Marek Zawirski, Annette Bieniusa, Valter Balegas, Sérgio Duarte, Carlos Baquero, Marc Shapiro et Nuno Preguiça en 2013
      - Dispo ici : https://arxiv.org/abs/1310.3107
      - Un rapport de recherche
    - *SwiftCloud: Fault-Tolerant Geo-Replication Integrated all the Way to the Client Machine*
      - Par Nuno Preguiça, Marek Zawirski, Annette Bieniusa, Valter Balegas, Sérgio Duarte, Carlos Baquero et Marc Shapiro en 2014
      - Dispo ici : https://ieeexplore.ieee.org/abstract/document/7000132
      - Un article de workshop si je pige bien
  - [X] Pour Redis
    - S'agit de la version Redis Enterprise qui propose un système de géo-réplication basé sur des CRDTs
    - Il n'y a pas l'air d'y avoir de papiers à ce sujet
    - Mais les pages webs suivantes :
      - https://redislabs.com/redis-enterprise/advantages/
      - https://www.redislabs.com/redis-enterprise/technology/active-active-geo-distribution/
  - [X] Pour Anna
    - Existe dorénavant 2 papiers sur Anna
    - *Anna: A KVS for Any Scale*
      - Par Chenggang Wu, Jose Faleiro, Yihan Lin et Joseph Hellerstein
      - Publié dans 2018 IEEE 34th International Conference on Data Engineering (ICDE)
      - Dispo ici : https://ieeexplore.ieee.org/document/8509265/
    - *Anna: A KVS for Any Scale*
      - Par Chenggang Wu, Jose Faleiro, Yihan Lin et Joseph Hellerstein
      - Publié dans IEEE Transactions on Knowledge and Data Engineering (Volume: 33, Issue: 2, Feb. 1 2021)
      - Dispo ici : https://ieeexplore.ieee.org/document/8640246
  - [X] Pour AntidoteDB
    - Y a pas l'air d'y avoir un papier directement sur AntidoteDB
    - Plusieurs papiers par contre se basent dessus
    - *Cure: Strong Semantics Meets High Availability and Low Latency*
      - Deepthi Devaki Akkoorath, Alejandro Z. Tomsic, , Manuel Bravo, Zhongmiao Li, Tyler Crain, Annette Bieniusa, Nuno Preguiça et Marc Shapiro
      - Publié à ICDCS'16
      - Dispo ici : https://ieeexplore.ieee.org/document/7536539
    - *Antidote SQL: Relaxed When Possible, Strict When Necessary*
      - Par Pedro Lopes, João Sousa, Valter Balegas, Carla Ferreira, Ségio Duarte, Annette Bieniusa, Rodrigo Rodrigues et Nuno Preguiça
      - Jamais publié ?
      - Dispo ici : https://arxiv.org/abs/1902.03576
  - Par contre, j'ai comme un doute, Shapiro recrutait pour AntidoteDB ou pour un autre projet ?
  - À re-vérifier
    - J'ai rien trouvé en cherchant rapidement
    - Faudrait que je demande à Claudia ou Gérald
  - Ah si c'est bien ça, il s'agissait du projet/de la start-up Concordant
    - https://www.concordant.io/
  - C'est vrai qu'il y a aussi Lasp
  - [X] Pour Lasp
    - *Lasp: a language for distributed, eventually consistent computations with CRDTs*
      - Par Christopher Meiklejohn et Peter Van Roy
      - Article de workshop publié à PaPoC'15
      - Dispo ici : https://dl.acm.org/doi/abs/10.1145/2745947.2745954
    - *Lasp: a language for distributed, coordination-free programming*
      - Par Christopher Meiklejohn et Peter Van Roy
      - Article de conf publié à PPDP'15
      - Dispo ici : https://dl.acm.org/doi/abs/10.1145/2790449.2790525
  - Et que Legion existe aussi !
  - [X] Pour Legion
    - *Legion: Enriching Internet Services with Peer-to-Peer Interactions*
    - Par Albert van der Linde, Pedro Fouto, João Leitão, Nuno Preguiça, Santiago Castiñeira, Annette Bieniusa
    - Article de conf publié à WWW'17
    - Dispo ici : https://dl.acm.org/doi/abs/10.1145/3038912.3052673
- Papier : Rédiger intro
  - Pourrait profiter de l'introduction des CRDTs pour expliquer qu'ils implémentent le modèle de cohérence SEC
    - Me permettrait de le retirer de *Proposed approach*
  - Plan
    - Systèmes distribués et réplication
      - Systèmes doivent répliquer les données afin d'assurer leur scalability, disponibilité, robustesse
      - Dans le cas de données mutables, la modification de ces données
      - Systèmes adoptent la stratégie de la réplication optimiste et du modèle de cohérence eventual consistency pour s'affranchir de ces coordinations synchrones
      - Mais nécessite un mécanisme de résolution de conflits pour gérer modifications concurrentes
    - CRDTs
      - Nouvelles spécifications des types de données abstraits
      - Conçus pour l'utilisation dans les systèmes distribués
      - Offrent une interface et sémantique similaire aux spécifications existantes pour les exécutions séquentielles
      - Mais définissent une nouvelle sémantique pour les nouveaux cas de figures issus d'exécutions parallèles
      - Respectent le modèle de cohérence SEC qui définit que l'ensemble des replicas convergent au même état lorsqu'ils reçoivent un même ensemble de modifications
	- Et ce sans communication supplémentaire requise
	- Peu importe leur ordre de réception
      - Intègrent un mécanisme de résolution de conflits
    - Utilisation des CRDTs
      - Depuis leur définition, les CRDTs ont été rapidement adopté par la littérature et l'industrie
      - De nos jours, on les retrouve
	- Des structures de données complexes pouvant servir de brique pour une variété d'applications (Yjs, JSON CRDT)
	- Utilisés dans les BDs distribuées (Riak, Redis, Anna, AntidoteDB)
	- Intégrés dans des langages de programmation (Lasp, CScript)
	- Keystone des Local-First Softwares (Local-First Software, PushPin)
    - CRDTs dans l'édition collaborative
      - Ont aussi été un centre d'intérêt et particulièrement étudié dans le cadre de l'édition collaborative
      - Dans ce domaine, le texte est représenté sous la forme d'une Sequence
      - Plusieurs travaux ont montré la pertinence de cette approche et des Sequence CRDTs pour ce domaine (Evaluating CRDTs for Real-time Document Editing, LSEQ)
      - Plusieurs éditeurs collaboratifs sont issus de ces travaux (Crate, MUTE)
      - Cependant, les structures données existantes souffrent d'un problème de croissance continue des métadonnées
    - Contribution
    - Déroulé du papier
- Papier : Réunion avec Olivier
  - Où j'en suis
    - A rédigé
      - Conclusion
      - Quelques lignes sur les Op-based CRDTs
      - Distinction entre modifications locales et distantes dans le cadre de l'évaluation
      - La mise à jour du modèle de livraison des opés pour de l'epoch-based
      - Abstract
    - Travaille actuellement sur l'intro
  - Ce qu'il reste à faire
    - J'avais noté quelques TODO?
    - Vu la longueur du document, je pense pas que ça soit intéressant de développer ces idées
      - Bon d'après la discussion avec Olivier, au moins rédigé celui sur la stratégie de renommage
    - Sinon, me reste le cas non-réciproque entre renameIdv2() et revertRenameId() à corriger
      - J'y avais déjà jeté un oeil lorsque je l'avais remarqué, sans succès
      - À voir si maintenant, en me replongeant dedans, j'arrive
  - Autres points
    - Souhaiterais prendre 2 semaines de vacs avant le <2021-03-15 Mon>
      - Donc le <2021-03-01 Mon> au plus tard
    - Mais souhaiterais finir l'article au préalable
      - Histoire d'avoir l'esprit tranquille
    - Permettrait de faire un break avant de se lancer dans la rédaction
  - Notes de réunion
    - Pas clair que l'arbre des epochs est une construction locale et indépendante
      - Peut comprendre qu'ils sont transmis/échangés entre les noeuds
      - Faudrait lever cette ambiguité
    - Ordre lexicographique
      - Est-ce que cet ordre peut poser problème ?
      - Donc non pour la convergence
      - Mais oui pour les performances pour converger
      - En parle dans discussion
    - Page 1 : Overhead
      - Préciser qu'il s'agit de l'overhead en mémoire, computation et bande-passante
    - Limites de LogootSplit
      - Expliciter qu'on peut se retrouver avec une séquence framgentée en blocs de 2-3 éléments et donc inefficace
      - Guider le lecteur vers la figure 12 pour appuyer la claim que 1% du doc est du contenu, 99% des métadonnées
    - Overview
      - P-e un peu trop expéditif, manque de liant entre les différents aspects abordés
      - Voir si peut retravailler ces différents paragraphes
    - Mécanisme de compression
      - Bien affirmer que cela ne modifie pas l'exécution de l'opération de rename
    - Figures des algorithmes
      - Dans le papier quand je fais référence à ces figures, devrait ajouter la mention que c'est un algo
	- "The algorithm of Figure..."
    - Algorithme
      - Pour le moment, les algos sont donnés et c'est au lecteur de se démerder avec
      - Ça serait cool d'avoir une explication qui accompagne l'algo pour bien montrer que l'on traite les différents cas possibles
      - En annexe ?
    - *As nodes have only a partial knowledge of the system and of the issued operations, they may.*
      - P-e un peu longue et incompréhensible comme phrase
      - Voir pour reformuler, décomposer et améliorer ce passage
    - Results
      - Mettre une intro à cette section plutôt que de démarrer direct
    - Sous-section sur la stratégie de renommage
      - Rédiger cette sous-section
  - Organisation
    - Je finis le papier comme je l'imagine
    - J'envoie cette version à Olivier une fois qu'elle est prête
    - En parallèle, j'essaie d'avoir les retours de Gérald
    - Pour les vacs, à voir comment ça avance sur ces 2 aspects
- À imprimer
  - Tant que je suis au LORIA, autant en profiter pour faire le plein de papiers
  - [X] *Optimistic replication*
    - https://dl.acm.org/doi/10.1145/1057977.1057980
  - [X] *Consistency Tradeoffs in Modern Distributed Database System Design: CAP is Only Part of the Story*
    - https://ieeexplore.ieee.org/abstract/document/6127847
  - [X] *A Critique of the CAP Theorem*
    - https://arxiv.org/abs/1509.05393
  - [X] *CScript: A distributed programming language for building mixed-consistency applications*
    - https://www.sciencedirect.com/science/article/abs/pii/S0743731520302896
  - [X] *Keeping CALM: When Distributed Consistency is Easy*
    - https://arxiv.org/abs/1901.01930
  - [X] *Local-first software: you own your data, in spite of the cloud*
    - https://dl.acm.org/doi/10.1145/3359591.3359737
  - [X] *PushPin: towards production-quality peer-to-peer collaboration*
    - https://dl.acm.org/doi/10.1145/3380787.3393683
  - [X] *Secure Conflict-free Replicated Data Types*
    - https://dl.acm.org/doi/abs/10.1145/3427796.3427831
- Meeting
  - Continue de travailler sur l'article
    - En train de finaliser l'intro
    - Aussi en train de compléter ma biblio
  - Réunion avec Olivier <2021-02-18 Thu>
    - M'a fait ses retours sur la dernière version que j'avais partagé, qui date de début de semaine dernière
    - Globalement, reste encore un peu de boulot d'édition
    - Mais on approche de la ligne d'arrivée
  - Plan
    - Je refais une nouvelle version en prenant en compte ses remarques
    - Et en ajoutant le contenu manquant (l'intro et une sous-section sur quand faire un renommage)
    - Et on refera à ce moment là une dernière passe (j'espère)
  - Faut maintenant que je vois Gérald
    - Normalement une réunion <2021-02-24 Wed> ou <2021-02-25 Thu>
  - Sinon, j'ai vu passer un article sur *Secure CRDT*
    - Par Manuel Barbosa, Bernardo Ferreira, João Marques, Bernardo Portela et Nuno Preguiça
    - Dispo ici : https://dl.acm.org/doi/abs/10.1145/3427796.3427831
    - Juste lu l'abstract, mais si je comprend bien, permettent d'appliquer des opérations à un CRDT chiffré
    - Je suppose que Claudia l'a vu passer, mais dans le doute je préfère le mentionner
  - Réunion
    - Claudia
      - Réunion avec directeur scientifique de Inria
      - Entretien pour faire le point sur nos recherches
	- 2 points principaux
	- François souhaite passer la main en chef d'équipe à Claudia
	  - Devrait nécessiter la rédaction d'un nouveau projet
	  - Mais p-e moyen de s'en passer
	- Pas eu de réponses concernant l'ADT MUTE
	  - Ont contacté la personne responsable
	  - Réponse finalement quelques jours pour nous dire que c'est refusé
	  - Ont fait remarqué que le volet sécurité était pas assez développé
	  - Autre point remarqué : manque de contact avec l'industrie
	  - Va prendre contact avec Valrich (pas sûr de l'ortho?) (Direction Inria des SI) pour voir si plan pour remplacer Notepad
      - Interview-profil pour la journée du 8 mars
      - Réunion avec Olivier et PA
    - Abir
      - Réunion pour les cours
      - Avance sur la thèse
	- Bientôt fini le chap 1 contributions
	- Démarre ensuite le chap 2 contributions
    - Alexandre
      - Quelques cours
      - A presque fini implem du parser/dump en BD
      - A obtenu les accès aux serveurs
      - Va pouvoir démarrer les traitements dessus
      - Doit faire une réunion avec Claudia pour discuter des métriques
    - Anis
      - Finit corrections du manuscrit sur chap 4-5 de la part d'Olivier
      - Entame les corrections du chap 6 de la part de François
    - Moi
    - Olivier
      - Réunion avec Matthieu
	- Commence à aborder le sujet du manuscrit aussi
      - Réunion avec Claudia et PA
      - Va commencer un mois de mars assez compliqué niveau cours
    - Pierre-Antoine
      - A préparé des scénarios
      - Réunion avec Claudia et Olivier
	- Au sujet des propriétés que l'on souhaite dans les éditeurs collaboratifs
** Semaine du <2021-02-22 Mon> au <2021-02-26 Fri>
*** Planned
**** DONE Papier : Rédiger intro
**** DONE Papier : Intégrer corrections et remarques proposées par Olivier
- Olivier m'a fait part de ses remarques et m'a listé des corrections à effectuer
- Modifier le papier en conséquence
**** DONE Papier : Rédiger sous-section sur stratégie de renommage
- Olivier juge intéressant/nécessaire la sous-section abordant la question de quand renommer
- La rédiger
**** DONE Papier : Envoyer cette version complète à Olivier et Gérald
- Une fois les tâches précédentes réalisées, on devrait obtenir une 1ère version complète de l'article
- La partager avec Olivier et Gérald pour obtenir des retours sur les nouvelles sections et sur l'article globalement
*** Done
- Papier : Intégrer corrections et remarques proposées par Olivier
  - [X] Pas clair que l'arbre des epochs est une construction locale et indépendante
    - Reformulation de la phrase pour mettre l'accent sur la construction de l'arbre à partir des opérations
  - [X] Overhead
    - Mention des différents types d'overhead dans l'intro
  - [X] Limites de LogootSplit
    - Modification du paragraphe de fin pour baser l'affirmation des metadonnées sur nos benchmarks
  - [-] Overview
  - [X] Mécanisme de compression
  - [X] Figures des algorithmes
    - Définition d'un environnement /myalgorithm/
  - [-] Algorithme
  - [X] *As nodes have only a partial knowledge of the system and of the issued operations, they may.*
    - Décomposition de la phrase en plusieurs pour guider pas-à-pas le lecteur
  - [X] Results
  - Me paraît pas trop mal comme ça
  - La description des algos me paraîtrait intéressant à ajouter, mais nécessite un peu de trop travail pour que je finisse ça cette semaine
  - Augmenterait aussi la taille de l'article qui me paraît déjà bien conséquent
  - Je préfère laisser ça en suspens pour le moment
- Papier : Rédiger sous-section sur stratégie de renommage
  - Je ne sais pas où mettre cette sous-section par contre
    - Pour le moment, l'avait mis à la fin de la *RLS without concurrent /rename/ operations*
    - Car il s'agissait du plus tôt auquel je pouvais aborder le sujet
    - Mais peut en fait garder cette discussion pour plus tard
    - Justement, peut ajouter cette sous-section à *Discussion*
    - Comme se place après la présentation de la gestion de conflits entre renommages concurrents et après évaluation
    - Permettrait d'avoir la discussion la plus complète sur le sujet
    - Et pouvoir explorer aussi l'objectif minimisation des renommages concurrents
    - Me parait plus adapté
    - Mais dans ce cas, nécessite de retravailler la conclusion lorsque j'aborde ce sujet
  - Plan
    - Insister sur le fait qu'il s'agit d'une opération système
    - Il s'agit donc aux designers du système de décider quand les noeuds doivent effectuer un renommage
    - Mais qu'il n'y a pas une solution unique
    - La solution dépend des contraintes du système et de ses noeuds
    - Pour prendre cette décision, plusieurs aspects à prendre en compte
    - L'aspect mémoire : quand devient-il intéressant de renommer ?
    - L'aspect computationel : quand renommer devient trop coûteux et peut impacter négativement l'expérience utilisateur ?
    - L'aspect réseau : quand l'opération de renommage deviendrait trop coûteuse à propager ?
    - Les résultats de notre évaluation présentés dans la section évaluation donnent une idée de l'évolution de chacune de ces métriques
** Semaine du <2021-03-15 Mon> au <2021-03-19 Fri>
*** Planned
**** DONE Projet Informatique : Préparer les réunions avec les différents groupes
- On avait fini la 1ère période du projet début février
- Celle-ci devait aboutir sur la réalisation du diagramme de classes et la définition de la liste des priorités des fonctionnalités
- Voir les repos de chaque groupe si à jour par rapport aux remarques de la dernière réunion
**** DONE CRDT : Lire *A higly-available move operation for replicated trees and distributed filesystems*
- A remarqué ce /draft/ sur la page de Kleppmann
- Peut être intéressant de voir comment leurs travaux ont avancé depuis PaPoC
- Disponible ici : https://martin.kleppmann.com/papers/move-op.pdf
**** CANCELLED Papier : Débugger /reverseRenameIdGreaterThanNewLastId()/
...
*** Done
- Projet Informatique : Préparer les réunions avec les différents groupes
  - Groupe Lucas Seznec - Axel Thouvenin - Gaston Maimbourg
    - Ci-dessous, les notes que j'avais prises concernant le travail demandé à l'issue de la dernière réunion
      - À faire d'ici la fin de la semaine
	- Ajouter les associations manquantes dans le diagramme
	  - Les associations de base, mais aussi celle d'héritage
	- Ajouter les différentes méthodes de chaque classe
	- Upload du diagramme de classes sur Git
	  - J'ai dit impérativement d'ici la fin de semaine
    - <2021-03-15 Mon> à 17h00, le diagramme de classes n'était toujours pas disponible sur leur repo Git
    - [X] Leur demander de l'uploader
    - Remarques
      - Gaston était celui qui animait le plus la dernière réunion
      - Garder ça en tête lors de la prochaine
    - [X] Les recontacter pour fixer la prochaine réunion
    - Pistes de travail pour cette semaine
      - Reviewer leur diagramme de classes une fois celui-ci disponible
	- Plutôt content de ce diagramme de classe
	- Il est plutôt propre, assez complet et donne matière à réflexion
	- Si j'ai un reproche à faire, c'est qu'il manque les paramètres des constructeurs et autres méthodes
	- La partie supérieure regroupent les classes ayant attrait aux personnages
	  - Me paraît bonne
	- La partie inférieure regroupent les classes ayant attrait au déroulement du jeu
	  - Me paraît plutôt bonne aussi
	- Quelques remarques
	  - *Partie* n'a pas de *Joueur*, ni de *Boss*
	  - Comment on détermine si un *Boss* est sur une case ?
	    - Information présente à la fois dans *Case* et dans *Boss*
	    - À laquelle se fier ?
	    - Si c'est les mêmes données, pourquoi dupliquer ?
	  - Comment est géré le déplacement du *Joueur* ?
	    - Pour le moment, /seDeplacer()/ dans *Joueur*
	    - Pas de notion de position, que ce soit dans *Joueur* et dans *Plateau*
	    - Je suppose qu'un *Joueur* ne peut pas de déplacer n'importe où
	      - Peut pas aller à la dernière case directement ou sortir de la map
	    - Doit donc y avoir une vérification à un moment donné du mouvement demandé
	    - Qui se charge de ça ?
	  - Un peu de mal à piger le fonctionnement de *Combat*
	    - Les méthodes /attaqueBasique()/, /attaqueSpe()/, c'est de qui ?
	      - *Joueur*, *Boss* ou les deux ?
	    - Qui se charge par exemple du calcul de dégâts infligé par l'attaque basique du *Joueur* ?
	    - Quelle différence entre l'attribut /mana/ de *Personnage* et les attributs /mana_joueur/ et /mana_ennemi/ présents dans *Combat* ?
	  - Les méthodes de *Recompense* me paraîssent étranges et pas forcément intuitives
	    - Quand on me parle de récompense, je pense à un composant passif
	    - Les méthodes /annoncer_XXX()/ me paraissent plus tenir de la responsabilité de *Partie*
          - Me pose la question si c'est *Plateau* qui doit gérer les combats et évènements
	    - Si par exemple c'est *Partie* qui doit déclarer la fin de la partie à l'issue d'un combat
      - Revoir avec eux l'objectif de la v1
      - Voir avec eux la répartition du travail
      - Reste la question de la BD en suspens
  - Groupe Thomas Champseix - Gabriel Masschelein
    - Ci-dessous, les notes que j'avais prises concernant le travail demandé à l'issue de la dernière réunion
      - À faire d'ici la fin de semaine
	- Mettre la dernière du diagramme au classe au format xml et png
	- Nettoyer le repo
	  ...
	  - Diagramme de classe au format de PDF
	  - Fichier notes.txt
    - <2021-03-15 Mon> à 17h30, le diagramme de classes vient d'être uploadé
    - Pistes de travail pour cette semaine
      - Démarrer l'implémentation du jeu en Python
      - Mettre à jour le diagramme de classes en fonction des problèmes rencontrés
    - Remarques
      - Diagramme de classes
	- Où/Par quel composant l'équipe de personnage est-elle créée et gérée ?
	- Même question pour le *Plateau de Jeu*
	- Un *Combat* n'offre pas de *Récompense* ?
	  - P-e simplement un choix douteux de noms de classe
        - Comment *Combat* connaît le *EquipeDePersonnages* qui affronte les ennemis ?
	- Un peu de mal à voir ce qui se passe lorsqu'un *Combat* se déroule vu l'absence de méthodes
	  - Tour du combat ?
	  - Comment ça se passe à la fin d'un combat ?
	  - Si l'équipe a perdu, que se passe-t-il ?
      - Code
	- Utilisent la librairie pygame
	  - Quelle version ?
	  - Serait intéressant si vous utilisez plusieurs librairies d'utiliser un gestionnaire de dépendance
	  - Poetry a l'air de faire le taf en Python
	    - https://python-poetry.org/
      - Mettre à jour README.md pour indiquer instructions sur comment installer et lancer le jeu
      - Des idées sur comment gérer la carte hexagonale ?
    - Notes dde réunion
      - Lundi
	- Remettre dans le bain
	- Complété le diagramme de classes
	- Démarré l'implémentation de votre jeu
- Papier : Débugger /reverseRenameIdGreaterThanNewLastId()/
  - Peut gérer ce cas en ajoutant la condition suivante
    #+BEGIN_BLOCK
Si lastId < newLastId      et
   id.length < 1           et
   newLastId.isPrefix(id)  et
   lastId < tail(id, 1)    et
   tail(id, 1) < newLastId
    #+END_BLOCK
  - Dans ce cas, on peut retourner /tail(id, 1)/
  - Reste à gérer les autres cas possibles
  - Puisqu'à l'epoch /e1/, /newLastId/ et /newId/ existent, il est possible d'insérer et générer un nouvel identifiant entre ces 2 là
  - En résulte un nouveau identifiant /newId2/ de la forme /newLastId + tail2/
    - Puisque généré entre /newLastId/ et /newLastId + id/ (newId)
  - /id ∈ ]lastId, newLastId[/
  - Donc /tail2 ∈ ]MIN_TUPLE, lastId[/
  - Peut gérer ce cas en ajoutant la condition suivante
    #+BEGIN_BLOCK
Si lastId < newLastId     et
   id.length < 1          et
   ...
   tail(id, 1) < lastId
    #+END_BLOCK
  - Dans ce cas, on peut retourner /lastId + MIN_TUPLE + tail(id, 1)/
  - Doit donc s'assurer à tester et gérer ces valeurs dans /renIdGreaterThanLastId()/
  - ~Par contre, me paraît pas possible de générer un identifiant /newId3/ de la forme suivante~
    - /id = newLastId + tail3/
    - Avec /tail3 ∈ ]newLastId, MAX_TUPLE[/
  - ~Si insère après /newId/, est censé générer un /newId3/ avec une base différente de /newLastId/~
  - Sauf si existe un identifiant qui a pour position la valeur suivante de celle de /newLastId/ et qu'on insère entre les deux
    - Possible ça ?
    - Si quelqu'un insère en concurrence au renommage à la toute fin du document, peut générer un tel identifiant
    - Peu probable, mais possible
  - J'ai l'impression que ce cas de figure finalement correspond au même cas que le cas habituel de /renIdFromPredId()/
  - Bon, perso je trouve que vouloir rendre ces fonctions réciproques est casse-gueule
    - Rend notamment la fonction /renameId()/ trop complexe pour gérer les /do/ et surtout /redo/
    - J'ai pas confiance en cette version de /renameId()/
    - Et en arrive à un niveau de complexité où je n'arrive plus à raisonner
  - Me paraît pas raisonnable de proposer cette version dans l'article
  - Je préfère rollback et assumer de fournir des fonctions non-réciproques
  - Et expliciter que cela ne pose pas de problème car la relation /priority/ définie prémunit des /redo/
  - Ce qui par contre me cloue sur place avec ma relation /priority/
  - Tant que je dois éviter les /redo/, je ne peux pas proposer une autre relation /priority/ minimisant les calculs à l'échelle du système
  - Une implémentation à base d'operation log permettrait par contre de contourner les /redo/
    - Puisque on utiliserait à chaque fois les infos des opérations initiales, et non plus l'état courant
  - Cela retirerait la contrainte actuelle bloquant le choix de la relation /priority/
- CRDT : Lire *A higly-available move operation for replicated trees and distributed filesystems*
  - Présente une validation formelle de leur algorithme
    - Réalisée en Isabelle/HOL
    - Valide que leur algo génère des arbres bien formés
      - Absence de cycle, une seule racine
    - Et que les replicas convergent
  - En dérive une implémentation Scala de leur Tree CRDT supportant insert/remove/move
    - Basée sur une Hash Table
    - Avec pour clé le noeud enfant
    - Et pour valeur le couple (noeud parent, metadonnées)
  - Effectue deux évaluations expérimentales de cette implémentation
  - La 1ère suit le scénario suivant
    - 3 pairs
    - Réplique chacun un arbre à l'aide d'un Tree CRDT
    - Initialement vide
    - Effectuent des opérations /move/ en utilisant des noeuds provenant d'un set de 1000 noeuds
      - Donc taille max de l'arbre est de 1k noeuds
    - Fait tourner l'expérience 10min
    - Mesure le temps d'intégration median de l'opération /move/ en local et remote
    - Fait varier le nombre d'opérations /move/ pour voir l'impact sur les perfs de 30 à 600 opés/s
      - À 600 opés/s, atteint 100% d'utilisation du CPU
      - Donc n'augmente pas le nombre d'opés par seconde au-delà de ce point
    - Temps d'insertion local constant aux alentours de 50-70us
    - Temps d'insertion remote croissant linéairement de 140us à 1.5ms
      - Augmente car de plus en plus d'opérations concurrentes introduisant un conflit
      - Donc nécessite de plus en plus d'undo et de redo
      - Précise pas par contre l'ordre de grandeur du nombre d'opés à undo/redo
  - La 2nde suit le scénario suivant
    - 3 pairs
    - 1 pair est désigné leader et se contente d'ordonner totalement les opérations qu'il reçoit et de les retransmettre aux autres
    - 2 pairs qui génèrent les opérations
    - Avec cette approche, application d'une opération est plus simple/efficace car pas de undo/redo
    - Permet au leader de dépasser le seuil de 600 opés/s
    - Traite jusqu'à 14k opés/s
    - Mais du point de vue des autres pairs, observe la latence des opérations car round-trip avec le leader
    - Plusieurs centaines de ms donc pour qu'une opération soit effectivement appliquée de leur point de vue
    - Rappelle aussi que l'approche CRDT se montre toujours /available/ même en cas de partition réseau
      - Contrairement à l'approche SMR qui bloquerait
    - Le leader se révèle être un SPOF aussi dans l'approche SMR
  - Remarques
    - Un peu de mal à déchiffrer l'évaluation comparative
      - Les mesures des temps d'intégration des CRDT local/remote operations ne prennent en compte que le temps d'execution de ces opérations
      - Alors que les mesures de temps d'intégration des operations dans l'approche State Machine Replication tiennent compte de la latence
	- Aussi de la partie sérialisation/désérialisation des messages non ?
      - Je comprends la comparaison, mais ça me paraît pas forcément cohérent
      - Notamment dans ce cas, pourquoi les mesures concernant les CRDT remote operations ne prennent pas en compte la latence ?
    - Dommage qu'il n'y ait pas de comparaison avec un autre Tree CRDT
      - Pour un peu comparer les overheads de différentes approches
    - Un peu curieux de l'évolution des perfs de la Hash Table avec un nombre plus important d'éléments
      - Mentionne notamment leur volonté de proposer un DFS
      - J'ai du mal à estimer le nombre d'éléments gérer généralement par ces applications
      - Et donc l'objectif de performance à atteindre
    - L'évaluation de l'approche CRDT ne se base que sur 3 pairs
      - Quid d'un plus grand nombre de pairs ?
      - Et donc d'un nombre plus grand d'opérations concurrentes ?
  - Réflexions
    - Ce travail légitime l'utilisation d'un operation log pour l'implémentation d'un mécanisme de résolution de conflits
    - Je pourrais proposer une telle approche pour RLS pour simplifier son fonctionnement et potentiellement améliorer ses perfs
- Meeting
  - Reviens de 2 semaines de vacs, donc la forme
  - Avant de partir, j'avais rédigé et envoyé une version complète de notre draft pour TPDS à Olivier et Gérald
  - J'attend dorénavant les retours là-dessus
  - En attendant, je me suis repenché sur mes fonctions de renommage
    - Pour rappel, je possède une fonction qui permet de mapper des identifiants d'une epoch à une suivante et une fonction qui permet d'annuler l'effet du renommage
    - Une propriété intéressante à avoir serait que ces 2 fonctions soient réciproques
      - Permettrait de me "balader" entre les epochs sans soucis
    - J'avais travaillé sur ce sujet le printemps/été dernier
    - Il y a de ça un petit moment, je me suis rendu compte que ce n'était pas le cas
    - Rencontre un problème pour des identifiants insérés après le renommage qui se trouvent dans une range donnée
    - Que le renommage est /undo/
      - Dans ce cas, je map l'identifiant vers un nouvel identifiant dans l'epoch précédente qui remplit les contraintes du mécanisme
    - Mais lorsque le renommage est /redo/, ne retombe pas sur les identifiants originels
    - J'avais déjà essayé de résoudre ce problème y a quelques mois : sans succès
    - Cette semaine, j'ai ré-essayé : sans succès de nouveau
    - Je raisonne dessus à la main, en écrivant des tests pour m'aider
    - Mais trop de cas et cas trop complexes
    - Autant mes versions initiales de mes fonctions qui se contentent de gérer le /do/ et le /undo/, j'ai à peu près confiance et je comprends encore ce que je fais
    - Autant les versions tentant de supporter le /redo/, je sais que c'est incomplet et j'ai peur d'avoir louper des trucs
    - Et le truc, c'est que j'ai pas forcément besoin de pouvoir /redo/
    - J'ai une relation, /priority/, qui définit comment je me déplace entre les epochs
    - Et la relation /priority/ que je propose actuellement, par nature, empêche de revenir sur une epoch
    - Donc je pense abandonner l'idée de définir des fonctions de renommage et d'inversion du renommage réciproques
    - Dans mon implem actuelle, ça me force à utiliser une relation /priority/ qui prémunit des /redo/
    - Mais au moins ça me paraît correct et permet d'avoir confiance en ce que je propose
    - Donc je vais retravailler les parties correspondantes du papier
      - Permettra de simplifier les algos présentés notamment et de gagner un peu de place là-dessus
    - Et ça me fait repenser à une autre implémentation du mécanisme de renommage que je considérais à un moment
      - L'implem actuelle se base sur l'état des noeuds
	- Un noeud applique un renommage sur son état courant
      - Se basant sur un operation log
	- Un noeud part de l'état résultant d'un renommage et réapplique les opérations concurrentes pour obtenir l'état équivalent à son état courant à la nouvelle epoch
      - Cette implémentation a le mérite d'éviter l'écueil du /redo/, car utilise les opérations originelles pour calculer l'état après un renommage
	- Comparé à l'implem actuelle qui utilise l'état courant, avec des identifiants ayant déjà subies des transformations
      - A aussi le mérite de changer la complexité de l'algo
	- Au lieu de dépendre du nombre d'éléments présents dans la liste comme actuellement
	- Dépendrait du nombre d'opérations concurrentes
	- Peut arguer que bien plus petit, surtout si je déclenche un renommage tous les 30k opérations
  - Réunion
    - François
      - Réunion avec Anis
      - A participé à une soutenance de thèse
      - Dispo toute la semaine pro sauf vendredi matin
      - Une VM pour MUTE est en cours
    - Abir
      - Continue sur la rédaction de son manuscrit
	- Plutôt bien semblerait
    - Alex
      - A terminé import XML
      - Travaille sur l'implem pour utiliser le dump provenant de wikipedia
	- Corrige des problèmes qui reste de l'implem
    - Anis
      - Réunion avec François
      - Effectue quelques corrections sur son algo
      - Corrige ses différents chapitres du manuscrit
    - Claudia
      - A eu réunion avec Alex
	- Progrès sur l'implémentation
	- Commence à appliquer les différentes métriques pour l'évaluation de la qualité
	- Alex a proposé une métrique
      - A rencontré Victorien ce matin avec Gérald
	- Aujourd'hui il va bien
	- Lui a proposé un CDD de 6 mois ou 1 an sur une approche que Claudia a commencé avec Weihai
	  - Sur budget de Fair & Smart
	  - Sur une BDD relationnelle distribuée utilisant des CRDTs en interne
	  - Il est intéressé par la proposition
	    - Semble pas chaud pour Areal par contre
	  - Mais ne veut pas commencer tout de suite
	    - Veut prendre 1 ou 2 mois pour lui
	  - Va voir avec Sophie pour proposer un contrat
      - A réunion semaine pro avec Jérôme Blanchard, PA, Olivier, Gérald et Alex
	- Pour discuter de l'avancement (de Fair & Smart ?)
      - Travaille pour des jurys de recrutement de CR
    - Gérald
    - Matthieu
    - Pierre-Antoine
      - A travaillé sur présentation pour réunion sur Fair & Smart
      - Fait un état de l'art par thématique sur les différents sujets potentiellement explorables
    - Quentin
      - A trouvé un CDI de Enseignant-Formateur au CESI de Orléans
      - Réunion avec Patricia Venturin
	- Au sujet de la condition des doctorants
** Semaine du <2021-03-22 Mon> au <2021-03-26 Fri>
*** Planned
**** DONE RLS : Étudier une implémentation du mécanisme de renommage basée sur un operation log
- Je galère à l'heure actuelle à définir des fonctions de renommage et d'inversion du renommage réciproques
  - J'ai ma fonction d'inversion du renommage qui annule correctement l'effet du renommage
  - Mais ma fonction de renommage ne renvoie pas forcément à l'identique un identifiant inséré après le renommage qui a ensuite été affecté par le undo
    - Lors d'un redo donc
- Cela ne pose pas de problème à l'heure actuelle avec ma relation /priority/ qui par définition empêche un redo
- Mais ça m'empêche de proposer une relation /priority/ plus efficace tant que je ne résous pas ce problème
- De plus, la complexité de mon mécanisme de renommage s'accroît avec le nombre d'éléments présents dans la séquence
  - Devrait pouvoir proposer une amélioration à une complexité en fonction du nombre de blocs
  - Mais ça rendrait encore l'algo plus imbitable qu'il ne l'est déjà actuellement
- Le papier de Kleppmann et al. sur une opération /move/ dans un arbre montre qu'un operation log est une solution viable
  - Le papier *A higly-available move operation for replicated trees and distributed filesystems*
- L'operation log aurait pour avantage de limiter les transformations effectuées qu'à des do et undo
  - On pourrait partir de l'état résultant du renommage prioritaire
    - Ce qui a l'avantage de bypasser les différents renommages intermédiaires entre l'epoch courante et la nouvelle epoch
  - Puis appliquer les opérations concurrentes présentes dans le log pour obtenir le nouvel état
  - Les opérations concurrentes proviendraient donc d'epochs antérieures ou concurrentes
    - À moins que je loupe quelque chose
  - Et comme on conserve l'opération initiale dans l'operation log, on peut repartir de celle-ci plutôt que de l'identifiant transformé présent dans l'état
- La complexité deviendrait donc fonction des opérations concurrentes
- Considérer et étudier plus en détail cette implémentation alternative
  - Notamment l'overhead spatial et temporel introduit par l'operation log
**** DONE CRDT : Lire *PushPin: Toward Production-Quality Peer-to-Peer Collaboration*
- Par Peter van Hardenberg et Martin Kleppmann en 2020
- Étudient et documentent leur développement d'une application Local-First
- Avec cette appli, ils souhaitaient aller plus loin que le POC
- Et donc rencontrer et évaluer toutes les difficultés que l'on peut rencontrer lors du dev de ce type d'applis
- Je souhaite voir s'ils abordent les problématiques de membership, d'accumulation des métadonnées, de troncage des données stables et de churn
- Dispo ici : https://martin.kleppmann.com/papers/pushpin-papoc20.pdf
**** DONE Distributed Systems : Lire *Consistency Tradeoffs in Modern Distributed Database Systems Design*
- Par Daniel J. Abadi en 2012
- Papier qui introduit PACELC, une extension du CAP theorem
- Je l'avais brièvement parcouru
- Le relire plus en détails
- Dispo ici : https://ieeexplore.ieee.org/abstract/document/6127847
**** TODO Papier : Revenir à une version non-réciproque des fonctions de renommage
- J'ai décidé d'abandonner la conception de fonctions de renommage et d'inversion du renommage réciproques
  - Puisque je n'y arrive pas
  - Et que j'en ai pas besoin tant que j'ai une relation /priority/ qui me prémunit de /redo/
- J'avais cependant rédigé l'article dans l'optique de proposer de telles fonctions
- Modifier le contenu concerné en conséquence
  - La section 5.3 doit aborder ce sujet
  - La section 7.3 doit dorénavant rappeler qu'il est nécessaire d'empêcher les /redo/ ou alors il est nécessaire de proposer des fonctions de renommage réciproques au préalable
  - Algo 4 du papier devrait sauter
**** TODO Papier : Introduire la notion de membership
- La stabilité causale requiert un protocole de membership pour pouvoir être déterminée
- Pour le moment, ne mentionne pas ce requirement
- Peut me l'être reproché et consiste en un angle d'attaque de mon approche
- Mettre les pieds dans le plat en référençant SWIM et Lifeguard comme protocole de membership scalable me paraît donc une meilleure option
**** TODO Papier : Relire *A Conflict-Free Replicated JSON Datatype* de Kleppmann
- A été publié dans TPDS
- Permettra de voir le format/les termes employés
- Dispo ici : https://arxiv.org/abs/1608.03960
**** TODO Papier : Relire *Context-Based Operational Transformation in Distributed Collaborative Editing Systems*
- A été publié dans TPDS
- Permettra de voir le format/les termes employés
- Dispo ici : https://ieeexplore.ieee.org/document/4668339
**** TODO RLS : Étudier le déclenchement de renommages identiques par les noeuds de manière non-coordonnée
- Pour le moment, adopte une stratégie "aggressive" pour renommer
- L'idée est que les noeuds génèrent une opération de renommage lorsqu'ils estiment cela nécessaire
- On espère que le nombre d'opés de renommage concurrentes restent minimes
- Et on gère ce cas de figure grâce aux mécanismes de résolution de conflits
- Une autre stratégie possible, que je souhaiterais étudier ici, est la stratégie "conservatrice"
- L'idée serait d'éliminer le risque de renommages concurrents
- Une piste évidente est l'utilisation d'un consensus synchrone
- Mais est-ce qu'il serait possible pour les noeuds de tous effectuer le même renommage, sans coordination synchrone ?
- L'idée serait qu'ils effectuent de manière indépendante un renommage sur le même set d'opérations, sur un même préfixe commun
- En ordonnant les opérations et en utilisant un mécanisme de stabilité, devrait être possible de définir ce préfixe
  - Intuitivement, je pense que PCS est requis
  - À vérifier si cela suffit réellement
  - Et si c'est le cas, si on peut relaxer les contraintes
- J'avais creusé cette question à un moment, mais me souviens plus des conclusions auxquelles j'étais arrivé
- Dans le doute, ça peut être intéressant de se repencher dessus
...
**** TODO P2P : Lire *Handling churn in a DHT*
- Par Sean Rhea, Dennis Geels, Timothy Roscoe, et John Kubiatowicz en 2004
- Une des faiblesses de RLS est la nécessité de stabilité causale pour pouvoir supprimer les métadonnées du renommage
- La notion de churn dans les systèmes P2P détruit totalement cette perspective
- Je cherche donc à me renseigner sur ce sujet pour notamment voir si des mécanismes ont été proposés pour atténuer ce problème
- Ce papier fait partie de ceux les plus cités sur le sujet
- Le modèle du système n'a pas l'air exactement de correspondre, mais peut toujours être intéressant
- Dispo ici : https://www2.eecs.berkeley.edu/Pubs/TechRpts/2003/CSD-03-1299.pdf
**** TODO P2P : Lire *Minimizing churn in distributed systems*
- Par P. Brighten Godfrey, Scott Shenker, et Ion Stoica en 2006
- Un autre papier sur le sujet du churn
- D'après l'abstract, j'ai l'impression que le papier ne concerne pas les systèmes P2P
  - Mais plutôt un système style Cloud
- Pas sûr qu'il soit très pertinent par rapport à ce que je recherche
- Dispo ici : https://dl.acm.org/doi/pdf/10.1145/1151659.1159931
**** TODO P2P : Lire *Understanding churn in peer-to-peer networks*
- Par Daniel Stutzbach et Reza Rejaie en 2006
- S'agit d'un papier qui cite *Handling churn in a DHT*
- Celui-ci a l'air de concerner les systèmes P2P
- P-e le plus pertinent par rapport à mes interrogations
- Dispo ici : https://dl.acm.org/doi/pdf/10.1145/1177080.1177105
**** TODO CRDT : Lire *Merging OT and CRDT Algorithms*
- Par Mehdi Ahmed-Nacer, Pascal Urso, Valter Balegas et Nuno Preguiça
- En reparcourant les différents papiers à la recherche de références à ajouter, je suis tombé sur celui-ci que j'avais loupé jusqu'à présent
- Suis curieux de voir s'il s'agit d'une architecture à la Legion ou d'une intégration de OT dans un CRDT
  - Ce qui se rapprocherait plus de ce que j'ai pu faire
- Dispo ici : https://hal.inria.fr/hal-00957167
**** TODO CRDT : Lire *Chronofold: a data structure for versioned text*
- Par Victor Grishchenko et Mikhail Patrakeev
- Présenté à PaPoC'20
- De mémoire, s'agissait d'une séquence répliquée construite à partir du log ordonné des opérations
- À 1ère vue, ne rentre pas dans une des approches habituelles pour concevoir les Sequence CRDTs
- À creuser
- Dispo ici : https://dl.acm.org/doi/10.1145/3380787.3393680
**** TODO CRDT : Lire *Near Real-Time Peer-to-Peer Shared Editing on Extensible Data Types*
- Par Petru Nicolaescu, Kevin Jahns, Michael Derntl, Ralf Klamma
- S'agit du CRDT implémenté par Yjs (https://github.com/yjs/yjs), qui est développé par Kevin Jahns
  - Qui est le framework présenté et évalué dans le blogpost *Are CRDTs suitable for shared editing?* (https://blog.kevinjahns.de/are-crdts-suitable-for-shared-editing/)
  - Par Kevin Jahns justement
- Me semble qu'on retrouve une structure similaire à WOOT/RGA pour représenter les Séquences
- Dispo ici : https://www.researchgate.net/publication/310212186_Near_Real-Time_Peer-to-Peer_Shared_Editing_on_Extensible_Data_Types
**** TODO CRDT : Lire *A string-wise CRDT algorithm for smart and large-scale collaborative editing systems*
- Par Xiao Lv, Fazhi He, Weiwei Cai, Yan Cheng
- A inspiré la notion d'item, l'équivalent des blocs, dans Yjs
- À lire
- Dispo ici : https://coek.info/pdf-a-string-wise-crdt-algorithm-for-smart-and-large-scale-collaborative-editing-sys.html
**** TODO CRDT : Lire *File system on CRDT*
- Par Mehdi Ahmed-Nacer, Stéphane Martin, Pascal Urso
- En reparcourant les différents papiers à la recherche de références à ajouter, je suis tombé sur celui-ci que j'avais loupé jusqu'à présent
- Vu que c'est une direction possible de mes travaux (utilisation de CRDT + OT pour concevoir une structure répliquée représentant un système de fichiers distribué), suis curieux
- N'a pas contre pas été publié/peer-reviewé
- Dispo ici : https://hal.inria.fr/hal-00720681
...
**** TODO Distributed Systems : Lire *Epoch-based Commit and Replication in Distributed OLTP Databases*
- Par Yi Lu, Lei Cao, Xiangyao Yu, Samuel Madden
- Si je pige bien, dans le contexte des transactions distribuées
- Propose d'aggréger des groupes de transactions dans des epochs
- Et d'utiliser les epochs comme unité de commit
- De façon à limiter le nombre d'exécutions du 2PC
- Dispo ici : http://www.vldb.org/pvldb/vol14/p743-lu.pdf
*** Done
- RLS : Étudier une implémentation du mécanisme de renommage basée sur un operation log
  - Plusieurs questions à creuser
  - Quel est le coût de l'algo pour identifier et récupérer les opérations concurrentes à une opération de renommage donnée présentes dans l'operation log ?
    - Implique de parcourir l'operation log
      - Complexité en fonction de sa taille
      - Après le parcours peut être parallelisé
      - Et le log tronqué au fur et à mesure pour garder une taille ok-ish
	- Besoin de stabilité pour cela
    - Pour chaque opération, comparer ses metadonnées à celles de l'opération de renommage
      - Vérifier si opération est présente dans le Version Vector de l'opération de renommage
      - Donc besoin d'un Version Vector pour l'opé de renommage
	- Problème de scalabilité
	- Ne porte cependant que sur l'opération de renommage qui est peu fréquente
	- Mais souffre néanmois du churn
	- Voir si des stratégies ont été proposées à ce sujet
    - Est-ce que peut me permettre de rejouer une opération passée ?
      - Je dirais que oui
      - Rejouer une suppression ne pose pas de problème
      - Rejouer une insertion en soi ne pose pas de problème
      - Dois juste éviter de rejouer une insertion après sa suppression
      - Les opérations sont délivrées dans un ordre valide d'application à la séquence répliquée
      - Vois mal comment une insertion pourrait être rejouée sans que la suppression correspondante ne le soit aussi
    - Est-ce que je peux réduire la précision de ma détection de concurrence et potentiellement rejouer des opérations en trop sans rendre incorrect l'état obtenu ?
      - Par exemple, pourrais me baser uniquement sur l'epoch pour identifier quelles opés jouer ?
	- Mouais
	- Des opérations provenant d'epochs précédentes peuvent être concurrentes au renommage
	- Des opérations provenant d'epochs d'une autre branche de l'epoch tree peuvent avoir été observé avant la génération du renommage
	- Me permet pas d'identifier les opérations à jouer
      - À creuser
  - Quand et comment peut-on tronquer l'operation log ?
    - Pourrais réutiliser les règles de GC introduites pour l'epoch tree
    - ~Peut pas juste supprimer les opérations observées par une opé de renommage devenue stable~
      - Vraiment ?
      - À partir de là, toutes les opés de renommage que l'on va recevoir auront observé ces opés
      - Donc plus besoin de les transformer par rapport à la nouvelle epoch pour les incorporer dans le nouvel état
    - Donc si, peut supprimer ces opérations là
    - Ne peut pas par contre supprimer l'opération de renommage elle-même
    - Pour ces opérations, retombent sur les règles présentées dans le papier
    - Comment déterminer qu'une opé de renommage est stable ?
      - Plusieurs façons de faire
      - Traquer en local l'epoch de la dernière opération reçue pour chaque noeud
	- Nécessite donc un vecteur dont la taille dépend du nombre de noeuds
	  - Mais uniquement local, jamais besoin de le partager
	  - À la connexion d'un nouveau pair p-e ?
	- À l'aide des Version Vectors associées aux epochs dans les opérations de renommage, peut générer un Version Vector minimal de stabilité
      - Mécanisme usuel : avoir une matrice de Version Vectors maintenue en local
	- Créée à partir des informations du mécanisme d'anti-entropie
	- Peut ensuite calculer le Version Vector minimal indiquant les opérations stables
- Réunion Papier <2021-03-23 Tue> avec Gérald
  - C'est bien
  - Pas de remarques en terme de contribution
  - Notion de nodes
    - Explicité dans le system model
    - Mais avant, pas forcément clair
  - Notion de total dense order
    - Faudrait p-e l'expliciter
  - *Intro*
    - Présenter dans l'intro les résultats obtenus
    - Pas besoin que ça soit chiffré
  - *Background*
    - Va un peu vite sur la notion de CRDT
    - Expliciter cette notion au préalable ?
  - *LogootSplit*
    - Dernier paragraphe pas forcément évident
    - Semble parler de la croissance des identifants alors que j'ai pas abordé le problème encore
    - Veut en fait parler des metadonnées par blocs plutôt que par élément
  - *Limits*
    - Plutôt que de rentrer dans le détail de la figure 10, juste dire qu'on le verra à ce stade
  - *Proposed approach*
    - Le fusionner avec l'intro
  - *Definition of the rename operation*
    - Indiquer que l'on étend les propriétés que doivent respecter les Sequence Operation-based CRDTs
    - Voir où est-ce qu'elles ont été définies
    - Le papier de Logoot-Undo a l'air d'aborder la question
  - Se voit <2021-03-25 Thu> 09:30 pour retravailler de l'intro jusqu'à la partie contribution
  - En attendant
    - Gérald relit le reste
    - Moi je commence les modifs sur les fonctions qui deviennent non-réciproques
- CRDT : Lire *PushPin: Toward Production-Quality Peer-to-Peer Collaboration* CRDT : Lire *PushPin: Toward Production-Quality Peer-to-Peer Collaboration*
  - Re-explique le principe de Local-First Software
  - Constate l'absence d'existence d'apps mainstream de ce type
  - But de ce projet est d'explorer la faisabilité d'une telle appli
    - Autant l'aspect technique (les problèmes rencontrés et comment les résoudre)
    - que l'aspect développeur (quelles difficultés vont-iels rencontrer ?)
  - Partent sur un projet simple
    - Un tableau répliqué qui permet aux différents utilisateurs d'y coller des notes, images, liens vers d'autres tableaux
    - De cette manière, l'absence de fonctionnalités comme l'access control ne pose pas trop de problèmes
  - Plusieurs points intéressants
  - Autorise la présence de serveurs dans l'infrastructure
    - Notamment pour la synchro et le back-up
    - Mais souhaite tout de même limiter leur rôle au maximum, pour les rendre les plus facilement remplaçables possible
    - Va dans le sens de ce qu'on a pu faire avec Pulsar
  - Considère les navigateurs pas prêts pour leur vision
    - APIs de stockage (localStorage et IndexedDB) lentes et perdant silencieusement des données
    - APIs réseau (WebRTC) reposant sur un serveur centralisé (signaling) et trop contraintes
      - Pas moyen d'utiliser des sockets TCP/UDP pour implémenter autre protocole P2P
    - Font donc le choix d'utiliser Electron
    - Est-ce que Web Assembly serait une autre solution pour répondre à ces problèmes mis en lumière ?
  - Considère Dat Protocol comme la stack réseau la plus adaptée à leurs besoins
    - Utilise /hypercore/, un append-only log
    - Nécessite un couple clé privée/publique
    - Log identifié à l'aide du hash de la clé publique
    - Clé privée nécessaire pour le modifier
    - Utilise donc un /hypercore/ pour chaque device qui modifie le document
    - Un document est donc mappé à un ensemble de /hypercore/
    - Indique qu'on peut retrouver les replicas d'un /hypercore/ à l'aide du hash de sa clé publique
    - Explique donc que l'url d'un document est construite en incorporant ce hash
    - Et qu'il s'agit alors d'un identifiant stable pour le doc, puisque va continuer de référencer l'/hypercore/
      - Même si son contenu a été modifié
    - Mais si un document est composé de plusieurs /hypercores/, quelle(s) URL(s) broadcast pour trouver des pairs ?
    - De ce que je comprends, broadcast toutes les URLs
      - Ce qui flood le réseau assez rapidement, comme les objets d'un document sont eux-mêmes des documents
    - Me paraît pas super intuitif pour les utilisateurs
      - Plusieurs URLs vont donc correspondre à un même document
      - Voir comment on partage un document
  - Liste en conclusion plusieurs problèmes ouverts
    - Comment faire cohabiter plusieurs versions différentes d'une même Local-First App ?
    - Comment gérer les droits d'accès ?
    - Comment montrer aux utilisateurs qu'iels possèdent des versions différentes d'un même document ?
  - Pas d'infos par rapport à ce que je cherchais
  - De ce que je comprends, le document ne peut que croître puisque construit à partir d'un ensemble de append-only log
    - Peut p-e au fur et à mesure qu'un pair avance juste maintenir l'état courant en local, et laisser des machines dédiées conserver l'intégralité des logs
  - N'aborde pas la question du churn
    - Si utilise un device une fois pour accéder et modifier le document, ajoute un /hypercore/
    - Celui-ci est ajouté à l'ensemble des /hypercores/ constituant le document
    - Ce qui introduit une nouvelle URL à broadcast lors de la peer discovery
- Distributed Systems : Lire *Consistency Tradeoffs in Modern Distributed Database Systems Design*
  - Motive le nouveau théorème en présentant une mauvaise compréhension du CAP Theorem
  - Argue que certains systèmes limitent notamment la Consistency même en cas de fonctionnement normal à cause de cela
  - Alors que le CAP Theorem indique que seulement en cas de partition réseau un choix doit être fait entre Consistency et Availability
  - Indique qu'un autre tradeoff est la véritable raison pour laquelle les systèmes diminuent les requirements niveau Consistency
  - Le tradeoff Latency-Consistency
  - Justifie cela en prenant pour exemples les différents Distributed Database Systems (DDBS) qui ont fait le choix de relaxer les contraintes sur la Consistency
  - Ces systèmes ont tous des uses cases incorporant des utilisateurs dans la boucle
  - La vitesse de réponse est donc essentielle pour ces derniers
  - Présente différents techniques de réplications, chacune proposant son tradeoff Latency-Consistency en cas de fonctionnement normal
    - Data updates sent to all replicas at the same time
      - Nécessite mécanisme de résolution de conflits
      - Protocoles de consensus maintiennent Consistency, mais au prix de Latency
      - Ne mentionne pas les CRDTs, mais rentreraient dans cette case
      - Minimise Latency, au prix de la Consistency
    - Data updates sent to an agreed-upon location first
      - Dépend de la technique de réplication
      - Si synchrone, assure Consistency mais augmente Latency
      - Si asynchrone, résultat dépend de comment le système traite les reads
	- Si tous les reads passent par le master, pas de réduction de la Consistency mais augmentation de la Latency
	- Si reads peuvent être répondus par tous les noeuds, privilégie Latency au prix de Consistency
      - Possible de mélanger ces 2 techniques de réplication pour atténuer les cas limites
    - Data updates sent to an arbitrary location first
      - Là encore dépend de la technique de réplication
      - Si synchrone, comme précédemment mais en plus de possible d'avoir de la latence supplémentaire en cas de conflits
      - Si asy,chrone, comme précéddemment
  - Afin de capturer les différents choix réalisés par les designers de DDBS, propose PACELC
  - Permet de décrire ce que le système privilégie en cas de de partition réseau (P) : Availability ou Consistency
  - Et de décrire ce que le système privilégie sinon (E) : Latency ou Consistency
* Backlog
